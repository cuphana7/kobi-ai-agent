...
function Install-KobiSkill([string]$Dir, [string]$Asset, [string]$Msg, [string]$SkipMsg, [string]$SkillsDir, [string]$InstallerRoot, [string]$TarExe) {
...
$Skills = @(
    @{ Dir='jennifer-monitor';    Asset='jennifer-monitor.skill';    Msg='제니퍼 APM 모니터링 스킬이 성공적으로 추가되었습니다.';       SkipMsg='안내: 제니퍼 APM 모니터링 스킬 자산이 없어 설치를 건너뜁니다.' }
    @{ Dir='project-bootstrap';   Asset='project-bootstrap.skill';   Msg='프로젝트 부트스트랩 스킬이 성공적으로 추가되었습니다.';         SkipMsg='안내: 프로젝트 부트스트랩 스킬 자산이 없어 설치를 건너뜁니다.' }
    @{ Dir='frism-cm';            Asset='frism-cm.skill';            Msg='Frism CM 연동 스킬이 성공적으로 추가되었습니다.';               SkipMsg='안내: Frism CM 연동 스킬 자산이 없어 설치를 건너뜁니다.' }
    @{ Dir='office-edit';         Asset='office-edit.skill';         Msg='Office(Excel/Word) 편집 스킬이 성공적으로 추가되었습니다.';     SkipMsg='안내: Office(Excel/Word) 편집 스킬 자산이 없어 설치를 건너뜁니다.' }
    @{ Dir='kbpay-service-check'; Asset='kbpay-service-check.skill'; Msg='KB Pay 서비스 지연 점검 스킬이 성공적으로 추가되었습니다.';     SkipMsg='안내: KB Pay 서비스 지연 점검 스킬 자산이 없어 설치를 건너뜁니다.' }
)

foreach ($sk in $Skills) {
    Install-KobiSkill -Dir $sk.Dir -Asset $sk.Asset -Msg $sk.Msg -SkipMsg $sk.SkipMsg `

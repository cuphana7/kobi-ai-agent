...
import {
  ...
  TOP_LEVEL_USAGE
} from "./chunks/chunk-HRPFBHW7.js";
import "./chunks/chunk-DAI65CMX.js";
import {
  initStartupProfiler
...
async function buildTopLevelHelpParser() {
  ...
  const parser = yargs([]).locale("en").scriptName("qwen").usage(TOP_LEVEL_USAGE).version("0.23.2").alias("v", "version").help().alias("h", "help").strict().demandCommand(0, 0);
...
async function printBootstrapVersion() {
  if ("0.23.2") {
    writeStdoutLine("0.23.2");
    return;
  }
  const { getCliVersion } = await import("./chunks/version-EK7VAGSI.js");
...
async function runCliEntry(rawArgv = process.argv.slice(2)) {
  ...
  if (managedUpdateVersion) {
    ...
    const { installManagedNpmUpdate } = await import("./chunks/managed-npm-update-VWBKHNIN.js");
  ...
  if (route === "serve") {
    const { tryRunServeFastPath } = await import("./chunks/fast-path-2X7BPMXX.js");
  ...
  ) ? await import("./chunks/acp-startup-profiler-KEU6VAYR.js") : void 0;
  acpStartupProfiler?.initializeAcpStartupProfiler();
  acpStartupProfiler?.markAcpStartup("geminiImportStart");
  const { main } = await import("./chunks/llm-2KPNSAVY.js");
...
async function handleCriticalError(error) {
  const [{ FatalError }, { AlreadyReportedError }] = await Promise.all([
    import("./chunks/deferred-core-runtime-Q7WFWYD3.js"),
    import("./chunks/errors-22ON46G7.js")
  ]);

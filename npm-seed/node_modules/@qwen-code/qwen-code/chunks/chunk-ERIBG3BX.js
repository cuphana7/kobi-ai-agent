...
var SchemaValidator = class {
   ...
   */
  static validateStrict(schema, data) {
    if (!schema || typeof schema !== "object" || Array.isArray(schema)) {
      return "schema must be a JSON object";
    }
    const strictAjv = isDraft2020Uri(
      schema.$schema
    ) ? new Ajv2020Class({ allErrors: true, strictSchema: true }) : new AjvClass({ allErrors: true, strictSchema: true });
    addFormatsFunc(strictAjv);
    try {
      const validate = strictAjv.compile(schema);
      return validate(data) ? null : strictAjv.errorsText(validate.errors, { dataVar: "value" });
    } catch (error) {

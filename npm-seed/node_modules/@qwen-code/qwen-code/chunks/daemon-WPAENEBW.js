...
var RestSseTransport = class {
  ...
  async *subscribeEvents(sessionId, opts = {}) {
    ...
    const fetchSignal = opts.signal ? composeAbortSignals([opts.signal, connectCtrl.signal]) : connectCtrl.signal;
    ...
    return await this.fetchWithTimeout(
      ...
      async (res) => {
        if (res.status === 200) {
          ...
          return typeof body.displayName === "string" ? { displayName: body.displayName } : {};
        }
...
var DaemonSessionClient = class _DaemonSessionClient {
  ...
  static async createOrAttach(client, req = {}, clientId) {
    ...
    const lastEventId = !session.attached || req.modelServiceId ? 0 : void 0;
...
function asKnownDaemonEvent(event) {
  switch (event.type) {
    ...
    case MID_TURN_MESSAGE_INJECTED_EVENT:
      return isMidTurnMessageInjectedData(event.data) ? event : void 0;
...
function isSessionMetadataUpdatedData(value) {
  return isRecord(value) && isNonEmptyString(value["sessionId"]) && isOptionalStringOrNull(value["displayName"]);
}
...
function isMidTurnMessageInjectedData(value) {
  return isRecord(value) && isNonEmptyString(value["sessionId"]) && Array.isArray(value["messages"]) && value["messages"].every((message) => typeof message === "string");
}
__name(isMidTurnMessageInjectedData, "isMidTurnMessageInjectedData");
...
function isSessionSnapshotData(value) {
  ...
  return (model === null || typeof model === "string") && (mode === null || typeof mode === "string");
}
...
function requireWorkspaceCwd(caps) {
  if (typeof caps.workspaceCwd !== "string" || caps.workspaceCwd.length === 0) {
    throw new DaemonCapabilityMissingError(
      "workspaceCwd",
      caps.workspaceCwd === "" ? "daemon returned an empty workspaceCwd (newer daemon with a bug)" : "daemon predates workspaceCwd support (1 daemon = 1 workspace); upgrade it"
    );
  }
...
function normalizeMidTurnMessageInjected(event, base) {
  ...
  const messages = Array.isArray(event.data["messages"]) ? event.data["messages"].filter(
...
function normalizeToolUpdate(update, base) {
  ...
  const title = getString2(update, "title") ?? toolName ?? toolKind;
...
function normalizePlanUpdate(update, base) {
  ...
  const planCallId = base.eventId !== void 0 ? `${DAEMON_PLAN_TOOL_CALL_ID}-${base.eventId}` : DAEMON_PLAN_TOOL_CALL_ID;
  const meta = isRecord2(update["_meta"]) ? update["_meta"] : void 0;
...
function createTextBlock(state, kind, text, eventId, serverTimestamp, meta) {
  return {
    id: allocateBlockId(state, kind),
    kind,
    text: truncateText(text),
...
function appendBoundedText(existing, text) {
...
function createDaemonTranscriptStore(seed = {}) {
  ...
  return {
    ...
    appendLocalUserMessage(text, images) {

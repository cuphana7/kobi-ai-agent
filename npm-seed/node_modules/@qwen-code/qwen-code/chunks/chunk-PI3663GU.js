...
function channelSettingsScope(workspaceCwd) {
...
__name(channelSettingsScope, "channelSettingsScope");
function channelRecordMap(value) {
...
__name(channelRecordMap, "channelRecordMap");
...
__name(snapshotFrom, "snapshotFrom");
...
var WorkspaceChannelSettingsStore = class {
  ...
  async upsert(name, options) {
    ...
    const envResolvableKeys = new Set(
      plugin.management.fields.filter((field) => field.envResolvable).map((field) => field.key)
    );
    ...
    for (const key of secretKeys) {
      ...
      if (update.operation === "preserve" && previousOnDisk[key] !== void 0 && envResolvableKeys.has(key)) {

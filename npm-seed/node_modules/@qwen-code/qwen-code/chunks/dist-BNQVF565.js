...
`);
  ...
  prepareGroupMessage(event, chatId, { forceAtMention } = {}) {
    ...
    const prefixSourceText = this.qqConfig.allowMention !== false ? safeDisplayText : safeCleanText;
    const strippedCommandText = configuredPrefix ? stripMessagePrefix(prefixSourceText, configuredPrefix) : safeCleanText;
    const rawCommandText = (strippedCommandText ?? safeCleanText).replace(/<@[^>]{1,64}>/g, "").trim();
    const isSlash = effectiveIsAtBot && strippedCommandText !== void 0 && rawCommandText.startsWith("/");
...
`);
    ...
    const payloadText = isSlash ? commandText : sanitizePromptText(strippedCommandText ?? prefixSourceText);
    const messagePrefixText = configuredPrefix && strippedCommandText !== void 0 ? `${configuredPrefix} ${payloadText}` : configuredPrefix ? prefixSourceText : void 0;
    const displayText = sanitizePromptText(isSlash && messagePrefixText ? messagePrefixText : prefixSourceText);
    const text = isSlash ? sanitizePromptText(messagePrefixText ?? safeCleanText) : `${head}${displayText}${suffixFromBotOpenId}`;
    const displayTextOffset = isSlash ? void 0 : head.length;
    return {
      ...
      text,
      displayText,
      ...displayTextOffset !== void 0 ? { displayTextOffset } : {},
      ...messagePrefixText !== void 0 ? { messagePrefixText } : {},
      senderName
    };
  }
  handleC2C(event) {
...
`);
    ...
    const configuredPrefix = this.configuredMessagePrefix();
    const strippedCommandText = configuredPrefix ? stripMessagePrefix(safeContent, configuredPrefix) : safeContent;
    ...
    const messagePrefixText = configuredPrefix && strippedCommandText !== void 0 ? `${configuredPrefix} ${commandText}` : configuredPrefix ? safeContent : void 0;
    const text = isSlash ? displayText : `[atMention=true] [${safeName}]: ${displayText}`;
    this.handleInbound({
...
`);
    ...
    if (!result)
      return;
    const { isSlash, text, displayText, displayTextOffset, commandText, senderName, safeName } = result;
    if (this.isDuplicate(event.id))
      return;
    if (isSlash) {
...
`);
    ...
    this.handleInbound({
      ...
      displayText,
      ...displayTextOffset !== void 0 ? { displayTextOffset } : {},
      ...result.messagePrefixText !== void 0 ? { messagePrefixText: result.messagePrefixText } : {},
      messageId: event.id,
      isGroup: true,
      isMentioned: true,
      isReplyToBot: true,
...
`);
    ...
    if (!result)
      return;
    const { isSlash, text, displayText, displayTextOffset, commandText, senderName, isAtBot, safeName } = result;
    if (!isAtBot) {
      if (this.groupActiveMsgEnabled.get(chatId) === false) {

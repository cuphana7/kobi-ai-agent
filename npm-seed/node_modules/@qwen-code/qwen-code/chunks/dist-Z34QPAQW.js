...
var GROUP_MSG_KEY = "sampleMarkdown";
...
`);
  ...
  supportsProactiveTarget(target) {
    return target.isGroup === true && target.threadId === void 0 && this.isConversationId(target.chatId);
  }
  /**
  ...
  async pushProactive(target, text) {
    ...
    const title = extractTitle(text);
    for (let i = 0; i < chunks.length; i++) {
      await this.sendProactiveChunk(target.chatId, i === 0 ? title : `${title} (cont.)`, chunks[i], `chunk ${i + 1}/${chunks.length}`);
    }
  }
  async getProactiveToken() {
    ...
    try {
      ...
      process.stderr.write(`[DingTalk:${this.name}] proactive send failed: token fetch error ${err}
`);
      throw new Error("DingTalk proactive send failed: could not fetch access token");
    }
    if (!data.access_token) {
      const errmsg = sanitizeLogText(String(data.errmsg ?? ""), 200);
      process.stderr.write(`[DingTalk:${this.name}] proactive send failed: gettoken errcode=${data.errcode} ${errmsg}
`);
      throw new Error(`DingTalk proactive send failed: gettoken errcode=${data.errcode}${errmsg ? ` ${errmsg}` : ""}`);
    }
    this.proactiveToken = {
  ...
  async sendProactiveChunk(conversationId, title, text, chunkLabel) {
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        process.stderr.write(`[DingTalk:${this.name}] proactive send error (${chunkLabel}): ${err}${cause ? ` (${cause})` : ""}
`);
      ...
      if (!resp.ok) {
        ...
        process.stderr.write(`[DingTalk:${this.name}] proactive send failed (${chunkLabel}): HTTP ${resp.status} ${detail}
`);

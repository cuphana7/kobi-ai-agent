...
var WorkspaceRuntimeCoordinator = class {
  ...
  stopping;
  get admissionPaused() {
    return this.draining || this.stopping !== void 0;
  }
  hasManagementWork() {
    return this.activeManagementOperations > 0 || this.skillsQueuedWork > 0 || this.mcpQueuedWork > 0;
  }
  beginStop() {
  ...
  scheduleSkillsReconciliation() {
    ...
    if (!snapshot.runtimeLive || this.admissionPaused || this.disposed) {
      this.skillsReconcileDeferred ||= snapshot.runtimeLive && this.admissionPaused;
    ...
    void this.queueSkillsWork(async () => {
      ...
      if (!this.admissionPaused && !this.disposed && revision === this.skillsRevision) {
  ...
  scheduleMcpReconciliation() {
    const snapshot = this.bridge.getWorkspaceRuntimeLifecycleSnapshot();
    if (!snapshot.runtimeLive || this.admissionPaused || this.disposed) {
      this.mcpReconcileDeferred ||= snapshot.runtimeLive && this.admissionPaused;
      this.mcpStatus = {
  ...
  async prepareSkillsRevision(revision) {
    ...
    try {
      ...
      if (revision !== this.skillsRevision) return;
      if (this.admissionPaused || !current.runtimeLive || current.runtimeEpoch !== runtimeEpoch || status.runtimeEpoch !== runtimeEpoch) {
  ...
  async prepareMcpRevision(revision) {
    ...
    try {
      ...
      while (true) {
        ...
        if (this.admissionPaused || this.disposed || !current2.runtimeLive || current2.runtimeEpoch !== epoch) {
          if (this.admissionPaused && !this.disposed && current2.runtimeLive && current2.runtimeEpoch === epoch) {
            this.mcpReconcileDeferred = true;
          }
  ...
  recordSkillsError(revision, runtimeEpoch, error) {
    const current = this.bridge.getWorkspaceRuntimeLifecycleSnapshot();
    if (revision !== this.skillsRevision) return;
    if (this.admissionPaused || !current.runtimeLive || current.runtimeEpoch !== runtimeEpoch) {
      this.skillsStatus = { state: "stale", revision, runtimeEpoch };
      return;
    }
    this.skillsStatus = {

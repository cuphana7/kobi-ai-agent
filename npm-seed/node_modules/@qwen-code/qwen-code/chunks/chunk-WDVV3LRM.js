    ...
    switch (type) {
      case "agent_message_chunk": {
        ...
        if (meta?.["qwenDiscreteMessage"] === true) {
          if (meta["source"] === "background_notification_response" && meta["rewritten"] !== true) {
...
`);
   ...
   */
  async replaceManagedWorktreeSession(sessionId, target, workspaceCwd, expectedCwd) {
    const bridge = this.bridge;
...
var NamedSessionManager = class {
  ...
  async resetWorktreeSession(task) {
    try {
      return await this.router.replaceManagedWorktreeSession(task.sessionId, task.target, this.cwd, task.cwd);
    } catch (error) {
...
`);
  ...
  async handleBtw(envelope, sessionId, question, sourceLabel) {
    const target = this.router.getTarget(sessionId);
    if (!target || target.channelName !== this.name) {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Could not resolve the current task for ${this.prefixedCommand("/btw")}.`, sourceLabel);
      return;
    }
...
`);
  ...
  async handlePermissionResponseCommand(envelope, args, decision) {
    ...
    if (lookup.kind === "ambiguous") {
      ...
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Multiple permission requests are pending for this chat. Reply with ${this.prefixedCommand(`/${decision} <request-id>`)}.
${requestList}`);
      return true;
    }
    if (lookup.kind === "none") {
    ...
    if (pending.userInputPresented && decision !== "deny") {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Submit this question through its interactive card, or use ${this.prefixedCommand("/deny [request-id]")} to cancel it.`, pending.sourceLabel);
      return true;
    }
    const response = (() => {
      if (decision === "deny") {
...
`);
  ...
  async worktreeRecoveryMessage(envelope, error) {
    ...
    const remedy = safeTaskName === void 0 ? "select this task first or close it" : `select this task first or close it with ${this.prefixedCommand(`/session close ${safeTaskName}`)}`;
...
`);
  ...
  async handleNamedSessionsCommand(envelope, args) {
    ...
    if (normalized !== "" && normalized !== "all") {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand("/sessions [all]")}`);
      return true;
    }
    try {
  ...
  async handleNamedSessionCommand(envelope, args) {
    ...
    try {
      switch (subcommand) {
        case "current": {
          ...
          await this.sendThreadMessage(envelope.chatId, envelope.threadId, current ? `Current task: ${current.name} (${current.isolation})` : `No task is currently selected. Use ${this.prefixedCommand("/session new <name>")} or ${this.prefixedCommand("/session use <name>")}.`);
          return true;
        }
        case "new": {
    ...
    await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand("/session current")} | ${this.prefixedCommand("/session new <name> [--worktree]")} | ${this.prefixedCommand("/session use <name>")} | ${this.prefixedCommand("/session close <name>")} | ${this.prefixedCommand("/session cancel [<name>]")}`);
    return true;
  }
...
`);
    ...
    const clearHandler = /* @__PURE__ */ __name(async (envelope, args) => {
      ...
      if (this.isSharedSession(envelope) && args.toLowerCase() !== "confirm") {
        await this.sendThreadMessage(envelope.chatId, envelope.threadId, `This clears the shared session for everyone who shares it. Re-send with "confirm" (e.g. ${this.prefixedCommand("/clear confirm")}) to proceed.`);
        return true;
      }
    ...
    this.registerCommand("help", async (envelope) => {
      const lines = [
        ...
        this.isSharedSession(envelope) ? `${this.prefixedCommand("/clear confirm")} \u2014 Clear the shared session (aliases: ${this.prefixedCommand("/reset")}, ${this.prefixedCommand("/new")})` : `${this.prefixedCommand("/clear")} \u2014 Clear your session (aliases: ${this.prefixedCommand("/reset")}, ${this.prefixedCommand("/new")})`,
      ...
      if (agentCommands.length > 0) {
        ...
        for (const cmd of agentCommands) {
          lines.push(`${this.prefixedCommand(`/${cmd.name}`)} \u2014 ${cmd.description}`);
        }
      }
      lines.push("", this.messagePrefix ? `Start each message with ${this.messagePrefix} to chat with the agent.` : "Send any text to chat with the agent.");
  ...
  async handleLoopCommand(envelope, args) {
    ...
    switch (subcommand.toLowerCase()) {
      ...
      default:
        await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand('/loop add "<cron>" <prompt>')} | ${this.prefixedCommand("/loop list")} | ${this.prefixedCommand("/loop inspect <id>")} | ${this.prefixedCommand("/loop cancel <id>")}`);
        return true;
    }
  }
  async handleLoopAdd(envelope, args) {
    ...
    if (!parsed) {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand('/loop add "<cron>" <prompt>')}`);
      return true;
    }
    try {
  ...
  async handleLoopInspect(envelope, id) {
    if (!this.loopController)
      return true;
    if (!id) {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand("/loop inspect <id>")}`);
      return true;
    }
    const jobs = await this.loopController.listForTarget(this.name, this.loopTargetFromEnvelope(envelope));
  ...
  async handleLoopCancel(envelope, id) {
    if (!this.loopController)
      return true;
    if (!id) {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand("/loop cancel <id>")}`);
      return true;
    }
    const jobs = await this.loopController.listForTarget(this.name, this.loopTargetFromEnvelope(envelope));
    const match = jobs.find((job) => job.id === id);
  ...
  async processInbound(envelope) {
    ...
    if (parsed) {
      ...
      if (parsed.command === "btw" && this.bridge.btw) {
        if (!this.isAuthorizedForSharedSession(envelope)) {
          await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Only authorized members can use ${this.prefixedCommand("/btw")} in this shared session.`);
          return;
        }
        btwQuestion = parsed.args.trim();
        if (!btwQuestion) {
          await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Usage: ${this.prefixedCommand("/btw <question>")}`);
          return;
        }
        ...
        if (envelope.imageBase64 || envelope.attachments?.length) {
          await this.sendThreadMessage(envelope.chatId, envelope.threadId, `${this.prefixedCommand("/btw")} supports text-only questions.`);
          return;
        }
      }
    }
...
`);
      ...
      if (!sessionId) {
        await this.sendThreadMessage(envelope.chatId, envelope.threadId, `No task is currently selected. Use ${this.prefixedCommand("/session new <name>")} or ${this.prefixedCommand("/session use <name>")}.`);
        return;
      }
    } else {
    ...
    if (this.namedSessions && !sourceLabel) {
      await this.sendThreadMessage(envelope.chatId, envelope.threadId, `Could not identify the selected task. Use ${this.prefixedCommand("/sessions")}, select it again, and retry.`);
      return;
    }

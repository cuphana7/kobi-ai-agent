...
function parseDaemonBackgroundTurn(value) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    return void 0;
  const record = value;
  if (typeof record["turnId"] !== "string" || !record["turnId"] || typeof record["taskId"] !== "string" || !record["taskId"] || record["kind"] !== "agent" && record["kind"] !== "monitor" && record["kind"] !== "shell" && record["kind"] !== "workflow" && record["kind"] !== "peer" || typeof record["startedAt"] !== "number" || !Number.isFinite(record["startedAt"]) || record["startedAt"] < 0 || ["toolUseId", "sourceTurnId", "label"].some(
...
var DaemonClient = class {
  ...
  workspaceProvidersInFlight = /* @__PURE__ */ new Map();
  promptLimit;
  ...
  async workspaceGitLog(limit, skip, range, options) {
    const params = new URLSearchParams();
   ...
   */
  requestWorkspaceProviders(path, label) {
...
var WorkspaceDaemonClient = class {
  ...
  workspaceGitLog(limit, skip, cwd, range, options) {
    const params = new URLSearchParams();
    if (limit != null) params.set("limit", String(limit));
    if (skip != null) params.set("skip", String(skip));
...
function isSessionClosedData(value) {
  return isRecord(value) && isNonEmptyString(value["sessionId"]) && isNonEmptyString(value["reason"]) && isOptionalStringOrNull(value["closedBy"]) && isOptionalNumberOrNull(value["exitCode"]) && isOptionalStringOrNull(value["signalCode"]) && (value["cause"] === void 0 || typeof value["cause"] === "string") && (value["persistenceUnconfirmed"] === void 0 || typeof value["persistenceUnconfirmed"] === "boolean");
}
...
function getFiniteNumber(value, key) {
  const entry = value[key];
  return typeof entry === "number" && Number.isFinite(entry) ? entry : void 0;
}
__name(getFiniteNumber, "getFiniteNumber");
function extractTranscriptTiming(update) {
  if (!isRecord2(update)) return void 0;
  const meta = isRecord2(update["_meta"]) ? update["_meta"] : void 0;
  const timing = meta && isRecord2(meta["timing"]) ? meta["timing"] : void 0;
  if (!timing) return void 0;
  const kind = timing["kind"];
  if (kind !== "request" && kind !== "tool") return void 0;
  const durationMs = getFiniteNumber(timing, "durationMs");
  if (durationMs === void 0 || durationMs < 0) return void 0;
  const startedAt = kind === "request" ? getFiniteNumber(timing, "startedAt") : void 0;
  const ttftMs = getFiniteNumber(timing, "ttftMs");
  const status = timing["status"];
  const toolStatus = timing["toolStatus"];
  const carriedStrings = {};
  const stringKeys = kind === "tool" ? ["responseId", "promptId", "subagentId", "callId", "toolName"] : ["responseId", "promptId", "subagentId", "model"];
  for (const key of stringKeys) {
    const value = getString2(timing, key);
    if (value !== void 0) carriedStrings[key] = value;
  }
  return {
    kind,
    durationMs,
    ...
    ...kind === "request" && ttftMs !== void 0 && ttftMs >= 0 ? { ttftMs } : {},
    ...kind === "request" && (status === "ok" || status === "error") ? { status } : {},
    ...kind === "tool" && (toolStatus === "success" || toolStatus === "error" || toolStatus === "cancelled") ? { toolStatus } : {}
  };
}
__name(extractTranscriptTiming, "extractTranscriptTiming");
...
function createDaemonToolResultPreview(output, content, opts = {}) {
  if (isShellResultDisplay(output)) {
    ...
    const limit = Math.floor(
      (MAX_TOOL_RESULT_PREVIEW_LENGTH - output.directory.length - output.outputFiles.join("").length) / count
    );
    return {
      kind: "shell_result",
      result: mapShellResultText(output, (value) => {
        ...
        const end = last >= 55296 && last <= 56319 ? limit - 1 : limit;
...
var SESSION_CLOSED_REASON_COPY = {
...
function normalizeDaemonEvent(event, opts = {}) {
  ...
  switch (event.type) {
    ...
    case "session_closed":
      if (isRecord2(event.data) && event.data["persistenceUnconfirmed"] === true) {
        return [
          {
            ...base,
            type: "error",
      ...
      return [
        {
          ...base,
          type: "status",
          text: getString2(event.data, "cause") === "workspace_runtime_stop" ? "Workspace runtime stopped." : SESSION_CLOSED_REASON_COPY[getString2(event.data, "reason") ?? ""] ?? "Session closed"
        }
      ];
    case "session_recording_degraded": {
...
// packages/sdk-typescript/src/daemon/shellResult.ts
init_esbuild_shims();
function isShellResultDisplay2(value) {
  return isShellResultDisplay(value);
}
__name(isShellResultDisplay2, "isShellResultDisplay");
...
export {
  ...
  isShellResultDisplay2 as isShellResultDisplay
};
/**
 * @license

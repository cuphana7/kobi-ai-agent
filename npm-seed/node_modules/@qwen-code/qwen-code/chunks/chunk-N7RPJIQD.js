...
var InboundGate = class {
  constructor(options) {
    ...
    this.answersForSeveralSessions = options.ownsSessionId !== void 0;
  }
  static {
  ...
  reevaluate(reason) {
    ...
    for (const entry of this.held) {
      if (dropMessagesForSessionsThatLeft && this.pinStatus(entry.frame) === "gone") {
        misaddressed += 1;
        this.settleMisaddressed(entry);
        continue;
      }
  ...
  pinStatus(frame) {
    try {
      ...
      debugLogger3.debug(
        `the pin check threw (leaving the message parked): ${error instanceof Error ? error.message : String(error)}`
      );
      return "unknown";
    }
  }
  ...
  expireOverdue() {
    ...
    for (const entry of this.held) {
      ...
      (expiryMs !== null && this.ageOf(entry) >= expiryMs ? expired : survivors).push(entry);
    }
    ...
    for (const entry of expired) {
      debugLogger3.debug(
        `held peer message ${entry.frame.msgId} expired after ${this.ageOf(entry)} ms (session ${entry.frame.toSessionId ?? "unpinned"} holds for ${this.getHeldExpiryMs(entry.frame.toSessionId)} ms)`
      );
      this.forgetAdmittedBody(entry.frame, originOf(entry));

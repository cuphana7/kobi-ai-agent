...
var inheritedDotEnvKeys = /* @__PURE__ */ new Set();
var inheritedSettingsEnvKeys = /* @__PURE__ */ new Set();
var inheritedProvenance = process.env[PRIVATE_RELAUNCH_ENV_PROVENANCE];
delete process.env[PRIVATE_RELAUNCH_ENV_PROVENANCE];
if (inheritedProvenance) {
  const sources = JSON.parse(inheritedProvenance);
  if (!sources || typeof sources !== "object" || !("dotEnv" in sources) || !("settingsEnv" in sources) || !Array.isArray(sources.dotEnv) || !Array.isArray(sources.settingsEnv) || !sources.dotEnv.every((key) => typeof key === "string") || !sources.settingsEnv.every((key) => typeof key === "string")) {
...
function getRelaunchEnvProvenance() {
  return {
    [PRIVATE_RELAUNCH_ENV_PROVENANCE]: JSON.stringify({
      dotEnv: [.../* @__PURE__ */ new Set([...inheritedDotEnvKeys, ...dotEnvSourcedKeys])],
      settingsEnv: [
        .../* @__PURE__ */ new Set([...inheritedSettingsEnvKeys, ...settingsEnvSourcedKeys])
      ]
    })
  };
}
__name(getRelaunchEnvProvenance, "getRelaunchEnvProvenance");
...
function findEnvFiles(settings, startDir, userLevelPaths = getUserLevelEnvPaths(), workspaceTrusted) {
  ...
  const workspaceIsTrusted = workspaceTrusted ?? isWorkspaceTrusted(settings, void 0, realStartDir).isTrusted === true;
  const canUseEnvFile = /* @__PURE__ */ __name((filePath) => {
    ...
    return isWorkspaceTrusted(settings, void 0, workspaceDir).isTrusted !== false;
  }, "canUseEnvFile");
...
function reportRejectedSettingsEnv(env, startDir) {
  const source = `settings.env (${startDir})`;
  reportRejectedLoaderKeys(source, Object.keys(env));
  if (Object.entries(env).some(
    ([key, value]) => key.toUpperCase() === "QWEN_AGENT_EXECUTION_BACKEND" && typeof value === "string" && value.trim() !== ""
  )) {
    writeStderrLineSafe(
...
__name(reportRejectedSettingsEnv, "reportRejectedSettingsEnv");

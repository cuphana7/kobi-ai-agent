...
var MAX_CONCURRENT_SUB_SESSIONS_PER_CALLER = 5;
var MAX_CONCURRENT_SUB_SESSIONS_TOTAL = 20;
...
function createSubSessionLauncher(opts) {
  ...
  const launch = /* @__PURE__ */ __name(async (info) => {
    ...
    if (current >= MAX_CONCURRENT_SUB_SESSIONS_PER_CALLER) {
      throw new Error(
        `Too many concurrent sub-sessions for this session (cap ${MAX_CONCURRENT_SUB_SESSIONS_PER_CALLER}); wait for one to finish.`
      );
    }
    if (inflightTotal >= MAX_CONCURRENT_SUB_SESSIONS_TOTAL) {
      throw new Error(
        `Too many concurrent sub-sessions in this workspace (cap ${MAX_CONCURRENT_SUB_SESSIONS_TOTAL}); wait for one to finish.`
      );
    }
      ...
      if (info.completion === "sent") {
        ...
        void (async () => {
          try {
            ...
            if (!(err instanceof Error && err.name === "AbortError") && !(stopAc.signal.aborted || drainAc.signal.aborted)) {
              log.debug(

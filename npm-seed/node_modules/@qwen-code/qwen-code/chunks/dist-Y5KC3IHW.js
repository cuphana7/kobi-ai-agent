...
var DwsChannel = class extends PollingChannelBase {
  ...
  constructor(name, config, bridge, options, client) {
    ...
    const messagePrefix = configuredString(config.messagePrefix, "messagePrefix");
...
`);
    ...
    if (this.findImTarget(chatId)?.kind === "direct") {
      await this.sendImText(chatId, this.formatAttributedText(text, label), idempotencyKey);
      return;
    }
    await this.client.replyToImMessage(chatId, messageId, senderId, this.formatAttributedText(text, label), idempotencyKey);
  }
  ...
  async admitReceivedMessage(source, message, fromHistory, reportFailure, admissionContext) {
    ...
    if (this.shouldFilterImMessage(source, message)) {
      if (source.kind === "group" || source.kind === "group-all") {
        const text = stripMessagePrefix(message.content.trim(), this.dwsMessagePrefix);
...
`);
  ...
  async admitMessage(source, message, key, reportFailure) {
    ...
    let remembered = true;
    if (!this.hasPendingMessage(key)) {
      const dispatchUnparkedAtCapacity = (source.kind === "at" || source.kind === "direct") && this.connected && (this.cursor.pendingMessages?.length ?? 0) >= MAX_PROCESSED_ITEMS;
...
`);
  ...
  async processImMessage(source, message, key) {
    const rawText = source.kind === "at" && this.requiresMention(message.conversationId) ? message.content.replace(/^\s*@[^\s\p{Cf}]+\s+(?=\/[a-zA-Z0-9_:-]+(?:\s|$))(?![\s\S]*(?<![A-Za-z0-9_])@)/u, "").trim() : message.content.trim();
    const providerDocumentNotification = source.kind === "direct" ? parseDocumentMentionNotification(rawText) : void 0;
    const text = providerDocumentNotification ? rawText : stripMessagePrefix(rawText, this.dwsMessagePrefix);
    if (this.dwsMessagePrefix && !text) {
      this.markProcessedMessage(key);
      this.saveCursor();
      return;
    ...
    const documentNotification = providerDocumentNotification ?? (source.kind === "direct" ? parseDocumentMentionNotification(text) : void 0);
...
`);
  ...
  async rememberPendingMessageWhenAvailable(source, message) {
    ...
    if (this.hasPendingMessage(key))
      return true;
    const generation = this.lifecycleGeneration;
    while (this.connected && generation === this.lifecycleGeneration && (this.cursor.pendingMessages?.length ?? 0) >= MAX_PROCESSED_ITEMS) {
    ...
    for (const pending of [...this.cursor.pendingMessages ?? []]) {
      ...
      if (this.shouldFilterImMessage(pending.source, pending.message)) {
        if (pending.source.kind === "group" || pending.source.kind === "group-all") {
          const text = stripMessagePrefix(pending.message.content.trim(), this.dwsMessagePrefix);
          if (!this.dwsMessagePrefix || text) {
            const envelope = this.createImEnvelope(pending.source, pending.message, text);

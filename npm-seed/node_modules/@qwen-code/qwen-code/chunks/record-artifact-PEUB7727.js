...
var DESCRIPTION = `Registers a session artifact so clients can show it in an artifacts panel. Use it after creating a useful file, URL, image, report, notebook, or other intermediate result that the user may want to open later.
...
function validateManagedId(value) {
  ...
  if (trimmed.includes("/") || trimmed.includes("\\") || trimmed.includes("..") || path.isAbsolute(trimmed) || path.win32.isAbsolute(trimmed)) {
...
function validateWorkspacePath(value) {
  ...
  if (portableNormalized === ".." || portableNormalized.startsWith("../") || path.posix.isAbsolute(portableNormalized)) {
...
function isArtifactKind(kind) {
  return kind === "file" || kind === "link" || kind === "html" || kind === "image" || kind === "video" || kind === "audio" || kind === "pdf" || kind === "notebook" || kind === "other";
}

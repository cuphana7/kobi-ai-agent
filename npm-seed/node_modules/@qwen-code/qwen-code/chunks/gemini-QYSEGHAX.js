...
var debugLogger3 = createDebugLogger("LSP_CONFIG_WATCHER");
var LspConfigWatcher = class _LspConfigWatcher {
  ...
  startWatching(listener) {
    if (this.started) return;
    debugLogger3.info(`Starting LSP config watcher for ${this.configPath}`);
    try {
  ...
  async stopWatching() {
    if (!this.started) return;
    this.started = false;
    debugLogger3.info(`Stopping LSP config watcher for ${this.configPath}`);
    if (this.refreshTimer) {
      clearTimeout(this.refreshTimer);
      this.refreshTimer = null;
    }
    ...
    try {
      ...
      debugLogger3.warn("LSP config watcher close error:", error);
    }
    this.watcher = void 0;
  }
  scheduleRefresh() {
    ...
    this.refreshTimer = setTimeout(() => {
      ...
      const activeDrain = this.drainPendingChange().catch((error) => {
        debugLogger3.warn("LSP config watcher refresh error:", error);
      });
  ...
  async handleChange() {
    ...
    debugLogger3.info(`LSP config changed: ${event.changeType} ${event.path}`);
  ...
  readSnapshot() {
    ...
    try {
      ...
      debugLogger3.warn("Failed to read .lsp.json:", error);
      return {
        exists: true,
    ...
    try {
      ...
      debugLogger3.warn("Failed to parse .lsp.json:", error);
      return {
        exists: true,
  ...
  async notifyListener(event) {
    ...
    try {
      ...
      debugLogger3.warn("LSP config change listener error:", error);
      return false;
    } finally {
      if (timerId !== void 0) clearTimeout(timerId);
...
var debugLogger4 = createDebugLogger("EXTENSION_FILE_WATCHER");
...
var ExtensionFileWatcher = class {
  constructor(config, extensionsDir = Storage.getUserExtensionsDir(), refreshState = new ExtensionRefreshState()) {
    this.config = config;
  ...
  startWatching() {
    ...
    if (roots.length > 0) {
      this.watcher = watch(roots, {
        ...
        debugLogger4.debug("Extension file event classified", {
          event,
          path: resolvedPath,
          action,
          marked
        ...
        debugLogger4.warn("Extension file watcher error:", error);
      });
    }
  ...
  stopWatching() {
    ...
    watcher?.close().catch((error) => {
      debugLogger4.warn("Extension file watcher close error:", error);
    });
    bootstrapWatcher?.close().catch((error) => {
      debugLogger4.warn("Extension bootstrap watcher close error:", error);
    });
  }
  restartWatching() {
    this.startWatching();
  ...
  watchExtensionsParent() {
    ...
    this.bootstrapWatcher = watch(parentDir, {
      ...
      debugLogger4.warn("Extension bootstrap watcher error:", error);
    });
  }
...
var debugLogger5 = createDebugLogger("WORKTREE_STARTUP");
...
// packages/cli/src/gemini.tsx
var debugLogger6 = createDebugLogger("STARTUP");
...
function installInteractiveSignalHandlers(wasRaw) {
  ...
  const handleSignal = /* @__PURE__ */ __name((signal) => {
    ...
    void runExitCleanup().catch((error) => {
      debugLogger6.error(`Error during ${signal} cleanup:`, error);
    }).finally(() => {
...
async function main() {
  ...
  if (argv.listExtensions) {
    ...
    const { handleList: handleListExtensions } = await import("./list-DEICDL65.js");
  ...
  const { themeManager, AUTO_THEME_NAME } = await import("./theme-manager-ZG4RY6GU.js");
...
${finalArgs[promptIndex + 1]}`;
  ...
  if (argv.resume !== void 0) {
    ...
    if (argv.resume === "") {
      const { showResumeSessionPicker } = await import("./StandaloneSessionPicker-WS4GXYOG.js");
  ...
  }
  {
    ...
    if (config.isInteractive() && !wasRaw && process.stdin.isTTY) {
      const { startEarlyInputCapture, stopAndGetCapturedInput } = await import("./earlyInputCapture-3VTHRC4C.js");
      const { detectAndEnableKittyProtocol } = await import("./kittyProtocolDetector-HCNNKJTI.js");
      ...
      if (!configuredTheme || configuredTheme === AUTO_THEME_NAME) {
        themeAutoDetectionComplete = themeManager.resolveAutoThemeAsync().catch((err) => {
          debugLogger6.warn("Async theme auto-detection failed:", err);
        });
      }
    }
    if (config.isInteractive()) {
      const { setMaxSizedBoxDebugging } = await import("./MaxSizedBox-6HYJ3ZPJ.js");
    ...
    const { initializeApp } = await import("./initializer-UTQ7FB7G.js");
    ...
    if (config.getExperimentalZedIntegration()) {
      const { runAcpAgent } = await import("./acpAgent-5FWSOZL5.js");
    ...
    if (config.isInteractive()) {
      ...
      const { startInteractiveUI } = await import("./startInteractiveUI-73EGLIIH.js");
...
${input}`;
    ...
    const { validateNonInteractiveAuth } = await import("./validateNonInterActiveAuth-EN6ZQSR2.js");
    ...
    if (inputFormat === "stream-json" /* STREAM_JSON */) {
      ...
      const { runNonInteractiveStreamJson } = await import("./session-HKAAI7BN.js");
    ...
    debugLogger6.debug(`Session ID: ${config.getSessionId()}`);
    const { runNonInteractive } = await import("./nonInteractiveCli-LS2K4X6A.js");
...
function registerLspHotReload(config, registerCleanup2) {
  ...
  lspConfigWatcher.startWatching(async (event) => {
    if (event.changeType === "invalid") {
      debugLogger6.warn(`Invalid LSP config file ${event.path}: ${event.error}`);
    ...
    try {
      ...
      debugLogger6.warn("Failed to reload LSP server settings:", error);
      if (!errorReported) {

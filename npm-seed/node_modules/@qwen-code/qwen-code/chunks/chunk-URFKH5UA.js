...
var EditToolInvocation = class {
  ...
  async getConfirmationDetails(abortSignal) {
    ...
    try {
      ...
      const errorMsg = isNodeError(error) && error.code === "EISDIR" ? `Target is a directory, not a file: ${this.params.file_path} (${error.code})` : getErrorMessage(error);
...
${snippetResult.content}`;
      ...
      return {
        ...
        error: {
          message: errorMsg,
          type: isNodeError(error) && error.code === "ESTALE" && this.config.getShellExecutionSandbox?.() ? "file_changed_since_read" /* FILE_CHANGED_SINCE_READ */ : isNodeError(error) && error.code === "EISDIR" ? "target_is_directory" /* TARGET_IS_DIRECTORY */ : "file_write_failure" /* FILE_WRITE_FAILURE */
        }
      };
    }
  }

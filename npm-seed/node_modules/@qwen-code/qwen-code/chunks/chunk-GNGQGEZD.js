...
function buildDeferredToolsReminder(toolRegistry) {
  ...
  if (!toolRegistry.getTool(ToolNames.TOOL_SEARCH) || !toolRegistry.getTool(ToolNames.TOOL_CALL)) {
    return null;
  }
...
function registerForkDisplayImageForCache(config, advertisedToolNames) {
  ...
  config.getToolRegistry().registerFactory(ToolNames.DISPLAY_IMAGE, async () => {
    const { DisplayImageTool } = await import("./display-image-WYMDGOWV.js");
...
function buildForkExecutionAllowlist(requestedTools, inheritedExecutionTools, advertisedTools = inheritedExecutionTools) {

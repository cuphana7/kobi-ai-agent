...
async function bootstrapServeFastPathEnvironment(workspace) {
  ...
  } = await import("./fast-path-settings-DAOVFADC.js");
  preResolveServeFastPathHomeEnvOverrides();
...
async function maybeOpenWebShellBrowser(handle, open) {
  ...
  const { maybeOpenWebShellBrowser: openBrowser } = await import("./serve-BT7KS6SC.js");
...
async function tryRunServeFastPath(rawArgv = process.argv.slice(2)) {
  ...
  const { runQwenServe } = await import("./run-qwen-serve-OZGR3FQA.js");
  let handle;
  try {

...
function partitionHeadlessToolCalls(requests, config) {
  return partitionByConcurrencySafety(requests, (request) => {
    ...
    return isToolCallConcurrencySafe(
      ...
      config.getToolRegistry().getTool(canonicalToolName(executionRequest.name))?.kind,
      executionRequest.args
    );
  });
}
__name(partitionHeadlessToolCalls, "partitionHeadlessToolCalls");
function getHeadlessExecutionRequest(request, config) {
  ...
  if (typeof targetName !== "string" || typeof targetArgs !== "object" || targetArgs === null || Array.isArray(targetArgs)) {
    return request;
  }
...
__name(getHeadlessExecutionRequest, "getHeadlessExecutionRequest");
async function runNonInteractive(config, settings, input, prompt_id, options = {}) {
  return promptIdContext.run(prompt_id, async () => {
    ...
    const cleanupReviewWorktrees = /* @__PURE__ */ __name((gitTimeout) => {
...
`
      ...
      const processToolCallBatch = /* @__PURE__ */ __name(async (batchRequests, setModelOverride, runtimeView) => {
        ...
        const executionRequestByResponse = /* @__PURE__ */ new Map();
        ...
        const launchToolCall = /* @__PURE__ */ __name(async (requestInfo) => {
          ...
          debugLogger.debug(
            `[runNonInteractive] tool call ${requestInfo.callId} (${executionRequest.name}) settled${response.error ? " with error" : ""}`
          );
          return response;
        }, "launchToolCall");
        const finalizeToolCall = /* @__PURE__ */ __name((requestInfo, toolResponse) => {
          const executionRequest = executionRequestByResponse.get(toolResponse) ?? getHeadlessExecutionRequest(requestInfo, config);
        ...
        const resolvedResponses = orderedResponses.map(
          ({ request, response }) => ({
            request,
            response,
            executionRequest: executionRequestByResponse.get(response) ?? getHeadlessExecutionRequest(request, config)
          })
        );
        ...
        for (let index = 0; index < resolvedResponses.length; index++) {
          const { request, response, executionRequest } = resolvedResponses[index];

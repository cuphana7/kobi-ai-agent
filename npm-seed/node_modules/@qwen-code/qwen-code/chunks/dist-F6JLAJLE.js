...
`);
  ...
  async connect() {
    ...
    this.bot.on("message:photo", async (ctx) => {
      const msg = ctx.message;
      const envelope = this.buildEnvelope(msg, msg.caption || "(image)", msg.caption_entities, false, !msg.caption);
...
(User sent an image but download failed)`;
    ...
    this.bot.on("message:document", async (ctx) => {
      ...
      const envelope = this.buildEnvelope(msg, msg.caption || `(file: ${fileName})`, msg.caption_entities, false, !msg.caption);
      this.prepareThenHandleInbound(envelope, async () => {
        try {
...
`);
          const promptText = this.configuredMessagePrefix() && msg.caption ? envelope.text : msg.caption || "";
          envelope.text = `${promptText}
...
`);
  ...
  startMessage() {
    ...
    return [
      ...
      "",
      prefix ? `Start each message with ${prefix} to chat with Qwen Code.` : "Send any message to chat with Qwen Code.",
  ...
  buildEnvelope(msg, text, entities, allowRegisteredCommandBypass = false, syntheticText = false) {
    ...
    const bypassMessagePrefix = allowRegisteredCommandBypass && !(configuredPrefix && startsWithMessagePrefix(text.trim(), configuredPrefix)) && (entities?.some((entity) => {
      ...
      if (atIndex !== -1 && value.slice(atIndex + 1) !== this.botUsername.toLowerCase()) {
        return false;
      }
      return TELEGRAM_BOT_COMMANDS.some((registered) => registered.command === command);
    }) ?? false);
    return {
      ...
      ...bypassMessagePrefix ? { bypassMessagePrefix: true } : {},
      isGroup,
      isMentioned,
      isReplyToBot,
      referencedText

...
`);
      ...
      entry.timer = setTimeout(() => {
        ...
        if (toFlush) {
          ...
          if (target) {
            this.sendMessage(target.chatId, toFlush).then(() => {
...
`);
                ...
                this.sendMessage(retryTarget.chatId, toFlush).then(() => {
                  entry.pendingRetry = "";
                  if (!entry.buffer && this.cronBuffer.get(sessionId) === entry)
                    this.cronBuffer.delete(sessionId);
                }).catch((retryErr) => {
...
`);
                    ...
                    this.sendMessage(retryTarget2.chatId, toFlush).then(() => {
                      entry.pendingRetry = "";
                      if (!entry.buffer && this.cronBuffer.get(sessionId) === entry)
                        this.cronBuffer.delete(sessionId);
                    }).catch((err2) => {
      ...
      if (raw.replyMsgId) {
        ...
        this.replyMsgId = new Map(Array.isArray(arr) ? arr.filter(([k]) => typeof k === "string" && k.length <= 256).filter(([, v]) => {
          ...
          if (v === null || typeof v !== "object")
            return false;
          const o = v;
          return typeof o["msgId"] === "string" && o["msgId"].length <= 128 && typeof o["timestamp"] === "number" && Number.isFinite(o["timestamp"]) && o["timestamp"] <= Date.now() + _QQChannel.REPLY_MSG_ID_TTL_MS;
        }).map(([k, v]) => [
          k,
...
`);
      ...
      if (raw.botOpenIdByGroup) {
        ...
        const totalRaw = Array.isArray(arr) ? arr.length : 0;
        this.botOpenIdByGroup = new Map(Array.isArray(arr) ? arr.filter(([k, v]) => typeof k === "string" && k.length <= 256 && typeof v === "string" && /^[A-F0-9]{32}$/i.test(v)) : []);
...
`);
  ...
  prepareGroupMessage(event, chatId, { forceAtMention } = {}) {
    const senderName = event.author?.username || event.author?.id || event.author?.member_openid || "QQ User";
...
\u673A\u5668\u4EBA OPENID: ${groupBotOpenId}` : "";
    const text = isSlash ? sanitizePromptText(safeCleanText) : `[atMention=${effectiveIsAtBot}]${openIdSuffix} [${safeName}${senderOpenId && /^[A-F0-9]+$/i.test(senderOpenId) ? `(${senderOpenId.slice(0, 8)}\u2026)` : ""}]: ${sanitizePromptText(this.qqConfig.allowMention !== false ? safeContent : safeCleanText)}${suffixFromBotOpenId}`;
    return {

...
import {
  require_axios,
  require_ws
} from "./chunk-A2X3KLC3.js";
import "./chunk-QXPI7FEC.js";
...
function getExplicitMention(body, botId) {
  const botSpecificMention = getBoolean(body, "isInAtList") ?? getBoolean(body, "is_in_at_list");
  if (botSpecificMention !== void 0)
    return botSpecificMention;
  const mentions = collectMentionValues(body);
  if (!mentions.present)
    return getBoolean(body, "isMentioned");
  return mentions.values.some((mention) => mention === botId || mention === "@all" || mention === "all");
}
__name(getExplicitMention, "getExplicitMention");
function collectMentionValues(body) {
  const values = [];
  let present = false;
  for (const key of [
    "mentions",
...
__name(collectMentionValues, "collectMentionValues");

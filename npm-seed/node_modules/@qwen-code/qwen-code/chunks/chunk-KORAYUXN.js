...
function createChannelHarness(options) {
  ...
  async function startIdleTimer(ci, context) {
    if (options.isRuntimeStopping() || ci.isDying || liveHarnessChannel() !== ci)
      return;
  ...
  function retireChannel(info, context) {
    info.isDying = true;
    cancelIdleTimer();
    keepAliveUntil = 0;
    info.channelLiveness?.stop();
  ...
  __name(retireChannel, "retireChannel");
...
function createSessionControlPlane(opts, createHarness) {
  ...
  let runtimeStopToken = randomUUID3();
  let lastRuntimeStop;
  let runtimeStop;
  function assertRuntimeNotStopping() {
    if (runtimeStop) throw new WorkspaceDrainingError(boundWorkspace ?? "");
  }
  __name(assertRuntimeNotStopping, "assertRuntimeNotStopping");
  let shutdownPromise;
    ...
    for (const sid of sessions) {
      ...
      try {
        sessEntry.events.publish({
          type: runtimeStop?.channel === info ? "session_closed" : "session_died",
          data: {
            sessionId: sid,
            reason: runtimeStop?.channel === info ? "client_close" : "channel_closed",
  ...
  const harness = createHarness({
    ...
    isRuntimeStopping: /* @__PURE__ */ __name(() => runtimeStop !== void 0, "isRuntimeStopping"),
  ...
  async function closeSessionImpl(sessionId, context, closeOpts) {
    ...
    try {
      ...
      if (closeOpts?.cause === "workspace_runtime_stop" && !agentSessionClosed) {
  ...
  function copyRuntimeStop(result) {
    return {
      ...result,
  ...
  __name(copyRuntimeStop, "copyRuntimeStop");
  function runtimeStopSnapshot() {
    const ci = liveChannelInfo();
    const blockedReasons = [];
    if (shuttingDown || runtimeStop || [...harness.values()].some((c) => c.isDying))
    ...
    if (harness.starting || inFlightSpawns.size || inFlightRestores.size || abandonedNewSessionSettlements.size || harness.runtimeOperationReservations || harness.pendingKeepAliveCount || inFlightSessionIdReservations.size || abandonedSessionIdReservations.size || ci && (ci.sessionSpawnsInFlight || ci.pendingRestoreIds.size))
      blockedReasons.push("session_start_pending");
    if (ci && (ci.harness.workspaceControlInFlight || ci.workspaceMcpDiscoveryInFlight || ci.workspaceMcpAuthenticationServerNames.size))
      blockedReasons.push("workspace_control_pending");
    if ([...byId.values()].some(
      (e) => isClosingOrAuthorizingClose(e) || resetPendingSessions.has(e.sessionId)
    ))
    ...
    return {
      ...
      sessions: [...byId.values()].map((entry) => {
        ...
        return {
          ...
          queuedPrompts: entry.pendingPromptList.filter((p) => p.state === "queued").length + entry.midTurnMessageQueue.length,
          isWaitingForPermission: summary.isWaitingForPermission === true,
          isWaitingForUserQuestion: summary.isWaitingForUserQuestion === true,
          ...summary.hasRunningBackgroundTasks !== void 0 ? { hasRunningBackgroundTasks: summary.hasRunningBackgroundTasks } : {}
        };
      }),
      ...lastRuntimeStop ? { lastStop: copyRuntimeStop(lastRuntimeStop) } : {}
    };
  }
  __name(runtimeStopSnapshot, "runtimeStopSnapshot");
  function stopWorkspaceRuntime(request, timeoutMs = 6e4) {
    if (lastRuntimeStop?.stopToken === request.expectedStopToken && lastRuntimeStop.channelId === request.expectedChannelId && lastRuntimeStop.runtimeEpoch === request.expectedRuntimeEpoch) {
      return runtimeStop?.promise ?? Promise.resolve(copyRuntimeStop(lastRuntimeStop));
    }
    const snapshot = runtimeStopSnapshot();
    const ids = snapshot.sessions.map((session) => session.sessionId).sort();
    if (request.confirmInterruptions !== true || snapshot.stopToken !== request.expectedStopToken || snapshot.channelId !== request.expectedChannelId || snapshot.runtimeEpoch !== request.expectedRuntimeEpoch || JSON.stringify(ids) !== JSON.stringify([...request.expectedSessionIds].sort())) {
      throw new WorkspaceRuntimeStopError("workspace_runtime_stop_stale");
    }
    if (snapshot.blockedReasons.length || !Number.isFinite(timeoutMs) || timeoutMs <= 0) {
      ...
      try {
        for (const sessionId of ids) {
          ...
          if (remainingMs <= 0) {
            ...
            receipt.error = "Session close budget exhausted; remaining sessions were not closed.";
            return copyRuntimeStop(receipt);
          }
          receipt.interruptedSessionIds.push(sessionId);
          try {
            await closeSessionImpl(sessionId, void 0, {
              cause: "workspace_runtime_stop",
        ...
        if (runtimeStop === operation) runtimeStop = void 0;
      }
    });
    let timer;
    operation.promise = Promise.race([
      ...
      new Promise((resolve2) => {
        timer = setTimeout(() => {
          receipt.state = "failed";
          receipt.error ??= "Workspace stop timed out before cleanup completed.";
          resolve2(copyRuntimeStop(receipt));
        }, timeoutMs);
        timer.unref?.();
      })
  ...
  __name(stopWorkspaceRuntime, "stopWorkspaceRuntime");
  startSessionReaper();
  ...
  const bridgeApi = {
    ...
    getWorkspaceRuntimeLifecycleSnapshot() {
      ...
      const stopping = runtimeStop !== void 0 || Array.from(channelInfos()).some(
      ...
      return {
        state: runtimeStop ? "stopping" : !runtimeLive ? stopping ? "stopping" : starting ? "starting" : "cold" : activeWork ? "active" : "idle",
        runtimeLive,
    ...
    getRuntimeStopCompletion: /* @__PURE__ */ __name(() => runtimeStop?.completion, "getRuntimeStopCompletion"),

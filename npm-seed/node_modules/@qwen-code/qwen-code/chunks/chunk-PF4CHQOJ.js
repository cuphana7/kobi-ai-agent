...
var COMPACT_GROUP_HORIZONTAL_PADDING = 2;
  ...
  for (const status of statuses) {
    for (let index = toolCalls.length - 1; index >= 0; index--) {
      ...
      if (tool.status === status) {
        ...
        return usesCountSummary ? safeDescription(tool.description) : void 0;
      }
    }
  }
  return void 0;
...
function estimateCompactToolGroupHeight(toolCalls, contentWidth) {
  ...
  const summaryWidth = Math.max(
    1,
    contentWidth - COMPACT_GROUP_HORIZONTAL_PADDING - STATUS_INDICATOR_WIDTH - getElapsedTimeReservedWidth(activeTool, overallStatus)
  );
...
var CompactToolGroupDisplay = /* @__PURE__ */ __name(({ toolCalls, contentWidth }) => {
  ...
  const isActive = isToolGroupActive(overallStatus);
  const hint = getActiveToolHint(toolCalls);
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(Box_default, { flexDirection: "column", width: contentWidth, paddingX: 1, gap: 0, children: [
          ...
          if (toolCall) {
            ...
            if (toolCall.status === "Success" /* Success */ && isCollapsibleTool(toolCall.name)) {
              toolCall.detailedDisplay = getToolResponseDisplayText(
                record.toolCallResult.responseParts ?? record.message?.parts
              );
            }
          }

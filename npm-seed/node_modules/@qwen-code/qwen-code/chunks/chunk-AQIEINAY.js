...
var ContentGenerationPipeline = class {
  ...
  async *processStreamWithLogging(stream, context, request, userPromptId) {
    ...
    const logPendingProtocolTagSanitized = /* @__PURE__ */ __name((response, sanitization) => {
      ...
      debugLogger4.warn("Sanitized a model protocol tag", {
        model: event.model,
    ...
    try {
      for await (const chunk of stream) {
        ...
        if (response.candidates?.[0]?.content?.parts?.length === 0 && !response.candidates?.[0]?.finishReason && !response.usageMetadata && // Preparation-only responses must reach ACP before arguments complete.

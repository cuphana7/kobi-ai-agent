...
async function truncateAndSaveToFile(content, fileName, projectTempDir, threshold, truncateLines, keep = "both", previewChars = threshold, exclusive = false) {
...
async function persistAndTruncateToolResult(callId, toolName, content, config) {
  ...
  const outputDirectory = config.getExecutionEnvironment?.()?.outputDirectory;
  try {
    const toolResultsDir = outputDirectory ?? config.storage.getToolResultsDir();
    ...
    if (outputDirectory !== void 0) {
      await fs3.writeFile(outputFile, content, { mode: 384, flag: "wx" });
    } else {
...
var MCP_SDK_REQUEST_TIMEOUT_CODE = "REQUEST_TIMEOUT";
function isMcpSdkRequestTimeout(error) {
  return error instanceof Error && error.name === "SdkError" && error.code === MCP_SDK_REQUEST_TIMEOUT_CODE;
}
__name(isMcpSdkRequestTimeout, "isMcpSdkRequestTimeout");
...
var DiscoveredMCPToolInvocation = class _DiscoveredMCPToolInvocation extends BaseToolInvocation {
  ...
  async loadMcpAppDisplay(toolResult, fallbackText, signal) {
    ...
    try {
      ...
      const reason = timeoutSignal.aborted || error instanceof Error && error.name === "TimeoutError" || isMcpSdkRequestTimeout(error) ? `resource read timed out (limit: ${timeoutMs} ms)` : cause;
...
var DiscoveredMCPTool = class _DiscoveredMCPTool extends BaseDeclarativeTool {
  constructor(mcpTool, serverName, serverToolName, description, parameterSchema, trust, nameOverride, cliConfig, mcpClient, mcpTimeout, mcpToolIdleTimeoutMs, annotations, alwaysLoad = false, allowInvocationContext = false, appResourceUri, appResourceUi) {
    super(
      ...
      true,
      // shouldDefer — MCP tools use ToolSearch + ToolCall to keep the

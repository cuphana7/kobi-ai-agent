...
import "./chunk-CNEIYWSA.js";
import "./chunk-HTHUX2T5.js";
import "./chunk-MXL5UP4X.js";
import "./chunk-3G7CIHVV.js";
import "./chunk-VA4KQY6V.js";
import "./chunk-NHDKSOEY.js";
import "./chunk-OQ4DRIOA.js";
import "./chunk-EMDMPBJ4.js";
...
import "./chunk-PODM4SZ4.js";
import "./chunk-6H67XLET.js";
import "./chunk-BAPGSC7U.js";
import "./chunk-P67PFTUT.js";
import "./chunk-57OAHC2Y.js";
import "./chunk-TO6QE22P.js";
...
import "./chunk-CCDMCZPP.js";
import {
  runExitCleanup
} from "./chunk-ECNYL4AA.js";
...
function filterAlwaysAllowOptions(confirmation, options, forceHideAlwaysAllow = false) {
  ...
  return hideAlwaysAllow ? options.filter((option) => option.kind !== "allow_always") : options;
}
...
var LlmRewriter = class {
  constructor(config, rewriteConfig) {
    ...
    if (rewriteConfig.promptFile) {
      const filePath = resolve2(rewriteConfig.promptFile);
...
var Session = class {
  ...
  isIdle() {
    return !this.pendingPrompt && !this.pendingPromptCompletion && !this.cronProcessing && !this.cronAbortController && !this.notificationProcessing && !this.notificationAbortController;
  }
  getTurnCount() {
    return this.turn;
  }
  ...
  dispose() {
    ...
    this.config.getBackgroundShellRegistry().setNotificationCallback(void 0);
  ...
  rewindToTurn(targetTurnIndex, opts) {
    ...
    if (this.pendingPrompt || this.cronProcessing || this.cronAbortController || this.notificationProcessing || this.notificationAbortController) {
      throw RequestError.invalidParams(
        void 0,
...
`
              ...
              while (nextMessage !== null) {
                ...
                try {
                  ...
                  for await (const resp of responseStream) {
                    ...
                    if (resp.type === "model_fallback" /* MODEL_FALLBACK */) {
  ...
  async #buildMidTurnParts(messages, abortSignal) {
    ...
    for (const message of messages) {
      ...
      try {
        ...
        debugLogger6.warn(`Failed to resolve mid-turn message: ${errorMessage}`);
      ...
      this.config.getChatRecordingService()?.recordMidTurnUserMessage(built, displayText);
      parts.push(...built);
    }
    return parts;
  }
  ...
  #registerBackgroundNotificationCallbacks() {
    ...
    this.config.getChatRecordingService()?.setTitleRecordedCallback((customTitle, titleSource) => {
  ...
  async #drainNotificationQueue() {
    ...
    try {
      ...
      this.notificationCompletion = null;
      void this.#drainCronQueue();
      if (this.notificationQueue.length > 0 && !this.pendingPrompt && !this.cronProcessing && !this.cronAbortController) {
        void this.#drainNotificationQueue();
      }
    }
  }
  ...
  async runToolCalls(abortSignal, promptId, functionCalls, toolLoopState) {
    ...
    const emitDuplicateBatch = /* @__PURE__ */ __name(async (batch) => {
      ...
      this.config.getChatRecordingService()?.recordToolResult(response.responseParts, {
        callId: response.callId,
        status: "error",
        resultDisplay: response.resultDisplay,
        error: response.error,
      ...
      return await runInToolSpanContext(toolSpan, async () => {
        ...
        try {
          ...
          const recordAutoModeFallbackResolution = /* @__PURE__ */ __name((outcome) => {
            if (approvalMode === "auto" /* AUTO */ && wasAutoModeDenialFallback && isApproveOutcome(outcome)) {
          ...
          if (!autoModeAllowed && needsConfirmation(confirmationPermission, approvalMode, toolName)) {
            ...
            if (approvalMode === "auto-edit" /* AUTO_EDIT */ && (confirmationDetails.type === "edit" || confirmationDetails.type === "info")) {
              ...
              const stopAfterPermissionCancel = /* @__PURE__ */ __name(() => {
                onStopAfterPermissionCancel?.();
                return earlyErrorResponse(
  ...
  #emitGoalStatusItems(result) {
    ...
    for (const item of result.outputHistoryItems ?? []) {
      if (item.type === "goal_status" /* GOAL_STATUS */) {
        this.emitGoalStatus({
          kind: item.kind,
          condition: item.condition,
          ...item.iterations !== void 0 ? { iterations: item.iterations } : {},
          ...item.setAt !== void 0 ? { setAt: item.setAt } : {},
          ...item.durationMs !== void 0 ? { durationMs: item.durationMs } : {},
          ...item.lastReason !== void 0 ? { lastReason: item.lastReason } : {}
        });
...
// packages/cli/src/acp-integration/acpAgent.ts
var debugLogger7 = createDebugLogger("ACP_AGENT");
...
var logWorkspaceMemoryExtractionError = createWorkspaceMemoryExtractionErrorLogger(debugLogger7);
function parseAcpLocalReadRootsEnv(raw = process.env[QWEN_ACP_LOCAL_READ_ROOTS_ENV]) {
  if (!raw) return [];
  return raw.split(path4.delimiter).map((entry) => entry.trim()).filter((entry) => entry.length > 0 && path4.isAbsolute(entry));
}
...
function buildAcpLocalReadRoots(config) {
  return [
    ...
    path4.join(config.storage.getProjectDir(), "subagents"),
...
function parseTranscriptReplayState(replay) {
  ...
  const pendingToolCalls = Array.isArray(rawPending) ? rawPending.filter(isPendingReplayToolCall) : [];
  if (Array.isArray(rawPending) && pendingToolCalls.length !== rawPending.length) {
...
__name(collectHistoryReplayUpdatesPage, "collectHistoryReplayUpdatesPage");
function liftSessionUpdateTimestamps(updates) {
  return updates.map((update) => {
    const record = update;
    const meta = record["_meta"];
    const timestamp = isObjectRecord(meta) ? meta["timestamp"] : void 0;
    return typeof timestamp === "number" || typeof timestamp === "string" ? { ...record, timestamp } : update;
  });
}
__name(liftSessionUpdateTimestamps, "liftSessionUpdateTimestamps");
...
function validateSkillSlug(slug) {
  if (!slug || slug === "." || slug === ".." || slug.includes("/") || slug.includes(path4.sep) || !/^[a-zA-Z0-9._-]+$/.test(slug)) {
...
function resolveSkillInstallPath(skillDir, relativePath) {
  const root = path4.resolve(skillDir);
  const target = path4.resolve(skillDir, relativePath);
...
function resolveManagedSkillDir(skillsBaseDir, slug) {
  const root = path4.resolve(skillsBaseDir);
  const skillDir = path4.resolve(skillsBaseDir, slug);
  if (!skillDir.startsWith(root + path4.sep)) {
    throw RequestError.invalidParams(void 0, "Invalid skill.slug");
  }
  return skillDir;
}
...
function readProviderSetupInputs(config, params, resolveExistingApiKey) {
  ...
  const apiKey = readOptionalString(params["apiKey"], "apiKey") ?? resolveExistingApiKey?.(protocol ?? config.protocol, baseUrl);
  if (!apiKey) {
...
function redactMcpServerSecrets(server) {
  const redactValues = /* @__PURE__ */ __name((record) => record ? Object.fromEntries(
...
function restoreRedactedMcpSecrets(server, existing) {
  const restore = /* @__PURE__ */ __name((incoming, prior) => {
...
async function resolvePreferredMemoryFile(dir, fallbackFilename) {
  ...
  return path4.join(dir, fallbackFilename);
}
...
async function runAcpAgent(config, settings, argv) {
  ...
  const shutdownHandler = /* @__PURE__ */ __name(async () => {
    ...
    try {
      ...
      debugLogger7.error("[ACP] Shutdown error:", err);
    } finally {
      eventLoopMonitor.dispose();
    }
    runExitCleanup().catch((err) => {
      debugLogger7.error("[ACP] Cleanup error:", err);
    }).finally(() => {
      process.exit(0);
    });
  }, "shutdownHandler");
...
var QwenAgent = class {
  ...
  discardStoredSessionIfCurrent(sessionId, session) {
    ...
    const logCleanupFailure = /* @__PURE__ */ __name((action, err) => {
      debugLogger7.debug(
        `Session ${sessionId} ${action} during failed restore cleanup failed: ${err instanceof Error ? err.message : String(err)}`
      );
    }, "logCleanupFailure");
    try {
      session.dispose();
  ...
  async loadSession(params) {
    ...
    const response = {
      ...
      models: availableModels,
      configOptions,
      ...sessionData?.artifactSnapshot ? { artifactSnapshot: sessionData.artifactSnapshot } : {}
    };
    if (!replayEnvelope) {
      return response;
    }
...
`
      ...
      return {
        ...
        servers: await Promise.all(
          Object.entries(servers).map(async ([name, server]) => {
            ...
            out.source = out.extensionName ? "extension" : workspaceSettings.mcpServers?.[name] ? "project" : "user";
  ...
  async buildWorkspaceSkillsStatus(config) {
    ...
    try {
      ...
      try {
        ...
        debugLogger7.warn("Extension cache refresh failed:", error);
      }
      try {
  ...
  buildWorkspaceToolsStatus(config) {
    ...
    try {
      ...
      const tools = registry.getAllTools().filter((tool) => !("serverName" in tool)).map((tool) => ({
        name: tool.name,
  ...
  async installSkillFromUrl(request) {
    ...
    const skillsBaseDir = path4.join(Storage.getGlobalQwenDir(), "skills");
    ...
    try {
      ...
      for (const file of download.files) {
        ...
        await fs2.mkdir(path4.dirname(targetPath), { recursive: true });
  ...
  async deleteGlobalSkill(request) {
    ...
    const resolvedSkillDir = path4.resolve(skillDir);
    const resolvedSkillFile = path4.resolve(skillFile);
    const globalDir = path4.resolve(Storage.getGlobalQwenDir());
    const isDedicatedSkillDir = resolvedSkillFile === path4.join(resolvedSkillDir, "SKILL.md");
    if (!isDedicatedSkillDir || resolvedSkillDir === path4.parse(resolvedSkillDir).root || resolvedSkillDir === globalDir) {
      throw RequestError.invalidParams(
        void 0,
  ...
  async readManagedSkillFile(slug, scope, skillManager, cwd) {
    if (scope === "global") {
      const qwenSkillDir = resolveManagedSkillDir(
        path4.join(Storage.getGlobalQwenDir(), "skills"),
        slug
      );
      const qwenSkillFile = path4.join(qwenSkillDir, "SKILL.md");
    ...
    return {
      skillDir: path4.dirname(skillFile),
      skillFile,
      content
    };
  }
  async findProjectSkillFileFromCwd(slug, cwd, skillManager) {
    ...
    for (const configDir of PROJECT_SKILL_DIRS) {
      const baseDir = path4.join(projectRoot, configDir, SKILLS_DIR);
  ...
  async setGlobalSkillEnabled(request, cwd) {
    ...
    if (path4.basename(skillFile) !== "SKILL.md") {
      throw RequestError.invalidParams(
        void 0,
      ...
      case SERVE_STATUS_EXT_METHODS.sessionTranscript: {
        ...
        try {
          ...
          return await runWithAcpRuntimeOutputDir(settings, cwd, async () => {
            ...
            let nextCursor;
            if (page.nextCursorState && replay.replayError === void 0) {
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMemoryRemember: {
        ...
        try {
          ...
          if (childSignal.aborted) {
            ...
            debugLogger7.error("Workspace memory remember timed out:", {
              projectRoot,
              code: timeoutCode,
          ...
          debugLogger7.error("Workspace memory remember failed:", {
            projectRoot,
            code,
            details: diagnostics.debugDetails,
            ...diagnostics.stack ? { stack: diagnostics.stack } : {}
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMemoryForget: {
        ...
        try {
          ...
          if (childSignal.aborted) {
            ...
            debugLogger7.error("Workspace memory forget timed out:", {
              projectRoot,
              code: timeoutCode,
              details: diagnostics.debugDetails,
              ...diagnostics.stack ? { stack: diagnostics.stack } : {}
          ...
          debugLogger7.error("Workspace memory forget failed:", {
            projectRoot,
            code,
            details: diagnostics.debugDetails,
            ...diagnostics.stack ? { stack: diagnostics.stack } : {}
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMemoryDream: {
        ...
        try {
          ...
          if (childSignal.aborted) {
            ...
            debugLogger7.error("Workspace memory dream timed out:", {
              projectRoot,
              code: timeoutCode,
              details: diagnostics.debugDetails,
              ...diagnostics.stack ? { stack: diagnostics.stack } : {}
          ...
          debugLogger7.error("Workspace memory dream failed:", {
            projectRoot,
            code,
            details: diagnostics.debugDetails,
            ...diagnostics.stack ? { stack: diagnostics.stack } : {}
...
`
        ...
        if (postStatus !== "connected" /* CONNECTED */) {
          throw new RequestError(
            -32099,
            `MCP server ${JSON.stringify(serverName)} did not reach a connected state after restart (status: ${postStatus}).`,
            {
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMcpManage: {
        ...
        if (action !== "enable" && action !== "disable" && action !== "authenticate" && action !== "clear-auth") {
          throw RequestError.invalidParams(
            void 0,
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionCd: {
        ...
        if (typeof targetPath !== "string" || targetPath.length === 0 || !path4.isAbsolute(targetPath) || targetPath.includes("\0")) {
          throw RequestError.invalidParams(
            void 0,
        ...
        let stats;
        try {
          stats = await fs2.stat(targetPath);
        } catch {
        ...
        const canonicalPath = await fs2.realpath(targetPath);
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionLanguage: {
        ...
        try {
          ...
          debugLogger7.warn("setLanguageAsync failed:", err);
          throw new RequestError(
            -32603,
        ...
        try {
          ...
          debugLogger7.warn("Failed to persist UI language setting:", err);
        }
        ...
        if (syncOutputLanguage) {
          ...
          try {
            ...
            debugLogger7.warn("Failed to write output-language.md:", err);
          }
          if (fileWriteOk) {
            try {
              this.settings.setValue(
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionRecap: {
        ...
        debugLogger7.debug(`recap ext-method received for session=${sessionId}`);
        const session = this.sessionOrThrow(sessionId);
        const config = session.getConfig();
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionBtw: {
        ...
        if (!cacheSafeParams) {
          debugLogger7.debug(`btw: no cacheSafeParams for session=${sessionId}`);
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionForkAgent: {
        ...
        try {
          ...
          debugLogger7.debug("Failed to read history before /fork:", error);
        }
        if (!hasHistory) {
          throw RequestError.invalidParams(
            void 0,
        ...
        try {
          ...
          debugLogger7.debug("Failed to record fork event in history:", error);
        }
...
\`\`\``
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeAdd: {
        ...
        if (typeof originatorClientId !== "string" || originatorClientId.length === 0) {
          throw RequestError.invalidParams(
            void 0,
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh: {
        ...
        await Promise.all([
          extensionManager.refreshCache().catch((err) => {
            debugLogger7.warn(
              `Extension refresh failed for session ${sessionId}: ${err instanceof Error ? err.message : String(err)}`
            );
          }),
          skillManager?.refreshCache().catch((err) => {
            debugLogger7.warn(
              `Skill refresh failed after extension refresh for session ${sessionId}: ${err instanceof Error ? err.message : String(err)}`
            );
          })
        ]);
        try {
      ...
      case SERVE_CONTROL_EXT_METHODS.sessionRewind: {
        ...
        if (promptId && (turnIndex === void 0 || turnIndex === null)) {
          const prefix = sessionId + "########";
        ...
        try {
          await session.getConfig().getChatRecordingService()?.flush();
          const cwd2 = session.getConfig().getProjectRoot();
          const sessionData = await runWithAcpRuntimeOutputDir(
            this.settings,
            cwd2,
            async () => {
              const sessionService = new SessionService(cwd2);
          ...
          artifactSnapshotUnavailable = err instanceof Error ? err.message : String(err);
          debugLogger7.warn(
            `[ACP] Failed to rebuild artifact snapshot after rewind for session=${sessionId}: ${artifactSnapshotUnavailable}`
          );
        }
        return {
          success: true,
          ...
          async () => {
            ...
            try {
              ...
              if (typeof name === "string" && name.trim().length > 0) {
                ...
                const stripped = existingTitle?.replace(/\s*\(Branch(?:\s+\d+)?\)\s*$/, "").trim();
...
`
      ...
      case "qwen/settings/setHook": {
        ...
        const hook = restoreRedactedHookSecrets(
          incomingHook,
          isReplace ? toRecord(eventHooks[index]) : {}
        );
        if (isReplace) {
  ...
  async getTranscriptReplayConfig(cwd, settings) {
    ...
    const pending = this.newSessionConfig(cwd, [], settings, void 0, false, {
      skipMcpDiscovery: true,
      skipHooks: true,
...
`
  ...
  async ensureAuthenticated(config) {
    ...
    try {
      ...
      debugLogger7.error(`Authentication failed: ${e}`);
      throw RequestError.authRequired(
        {

  ...
  async request(path, method, body, templateId) {
    for (let attempt = 0; ; attempt++) {
      ...
      if (!response.ok) {
        ...
        throw new DingtalkCardRequestError(`DingTalk Card OpenAPI ${method} ${path} failed${templateId ? ` for ${templateId}` : ""}: HTTP ${response.status}${detail ? ` ${detail}` : ""}`, isRetryableStatus(response.status));
      }
...
var DingtalkInteractionPresenter = class {
  ...
  registerRun(runId, ownerId, target, sessionId = "", sender, sourceLabel) {
    this.runs.set(runId, {
      runId,
      ownerId,
      target,
  ...
  appendOutput(segment, chunk) {
    ...
    void this.enqueue(run, () => {
      this.options.statusCards?.replace(statusContext, this.cardTarget(statusContext.target), this.withSourcePrefix(run, presentation.content));
    });
  }
    ...
    return this.enqueue(run, async () => {
      ...
      if (reason === "response_boundary") {
        ...
        if (!fallbackText2 || !this.options.sendFallback)
          return false;
        await this.sendFallback(run, presentation.context.target.chatId, fallbackText2, presentation.context.sessionId);
  ...
  terminalizeRun(runId, terminal, detail = "") {
    ...
    const finalization = this.enqueue(run, async () => {
      if (terminal === "failed") {
        ...
        const statusContext = run.statusContext;
        if (statusContext) {
          await this.options.statusCards?.complete(statusContext.segmentId, "", (retained) => retained ? this.withSenderPrefix(run, this.withoutRenderedSourcePrefix(run, retained)) : retained);
        }
      }
    });
...
var BACKGROUND_RESPONSE_AGGREGATION_TIMEOUT_MS = 10 * 60 * 1e3;
var BACKGROUND_RESPONSE_AGGREGATION_RETRY_MS = 30 * 1e3;
var BACKGROUND_RESPONSE_AGGREGATION_MAX_RETRIES = 3;
...
var DingtalkChannel = class _DingtalkChannel extends ChannelBase {
  ...
  constructor(name, config, bridge, options) {
    ...
    const rawAggregateBackgroundAgentResponses = config.aggregateBackgroundAgentResponses;
    if (rawAggregateBackgroundAgentResponses !== void 0 && typeof rawAggregateBackgroundAgentResponses !== "boolean") {
      throw new Error(`Channel "${name}" aggregateBackgroundAgentResponses must be a boolean.`);
    }
    this.aggregateBackgroundAgentResponses = rawAggregateBackgroundAgentResponses === true;
...
`);
        if (failOnHttpError) {
          throw new Error(`DingTalk reply send failed: HTTP ${resp.status} ${detail}`);
        }
...
`);
    this.disconnectDrain = Promise.allSettled(reactionStates.map((state) => state.tail)).then(() => void 0);
  }
  waitForDisconnect() {
...
`);
  ...
  onTaskLifecycle(event) {
    if (event.type === "started") {
      ...
      if (event.runId && event.owner && inboundOwner?.ownerId === event.owner.id) {
        ...
        this.interactionPresenter?.registerRun(event.runId, event.owner.id, inboundOwner.target, event.sessionId, inboundOwner.sender, ...sourceLabel ? [sourceLabel] : []);
  ...
  async dispatchBackgroundResponse(sessionId, text, context) {
    ...
    if (!target || target.channelName !== this.name || context !== void 0 && context.kind !== "agent") {
      return super.dispatchBackgroundResponse(sessionId, text, context);
    }
    const canAggregate = this.aggregateBackgroundAgentResponses && context?.kind === "agent" && typeof context.turnComplete === "boolean";
    if (!canAggregate) {
      if (text.trim().length === 0) {
        return super.dispatchBackgroundResponse(sessionId, text, context);
      }
      return super.dispatchBackgroundResponse(sessionId, this.formatBackgroundAgentResponse(text, context?.label), context);
    }
    ...
    if (!current && text.trim().length === 0) {
      ...
      if (context.turnComplete) {
        ...
        if (parked.resolvers === 0 && !parked.retryTimer && !parked.retryInFlight && (parked.retryAttempts ?? 0) >= BACKGROUND_RESPONSE_AGGREGATION_MAX_RETRIES) {
    ...
    if (!current) {
      ...
      try {
        ...
        if (parked.retiring || this.pendingBackgroundResponseTerminals.get(key) !== parked) {
  ...
  async flushBackgroundResponseAggregation(key, aggregation) {
    if (this.backgroundResponseAggregations.get(key) !== aggregation && !this.detachedBackgroundResponseAggregations.has(aggregation) || aggregation.flushing) {
      return;
    }
    ...
    if (!delivery) {
      ...
      delivery = {
        ...
        parts,
        partial: this.isPartialBackgroundResponseDelivery(aggregation, parts.length),
        attempts: 0,
    ...
    try {
      if (delivery.proactivePlan) {
        await this.deliverProactiveText(aggregation.target, delivery.proactivePlan);
      } else if (delivery.replyPlan) {
        await this.deliverReplyText(aggregation.target.chatId, delivery.replyPlan, true);
      } else if (this.supportsProactiveSend() && this.supportsProactiveTarget(aggregation.target)) {
        await this.deliverBackgroundResponseToTarget(aggregation.sessionId, body, {
          target: aggregation.target,
          sourceLabel: aggregation.sourceLabel
        });
      } else {
        delivery.preparedReplyBody ??= await this.prepareReplyOutput(aggregation.target.chatId, body);
        await this.deliverBackgroundReply(aggregation.target.chatId, delivery.preparedReplyBody, aggregation.sessionId, aggregation.sourceLabel, true, true);
      }
    } catch (caught) {
      error = caught;
    ...
    if (error === void 0) {
      ...
      if (delivery.composedTurnComplete === true && aggregation.turnComplete && delivery.partial !== true && !aggregation.retiring) {
    ...
    delivery.attempts++;
    process.stderr.write(`[DingTalk:${this.name}] background response delivery failed (attempt ${delivery.attempts}): ${sanitizeLogText(error instanceof Error ? error.message : String(error), 300)}
`);
    if (delivery.attempts >= BACKGROUND_RESPONSE_AGGREGATION_MAX_RETRIES || // A permanently rejected send (an invalid appkey, a missing app) cannot
   ...
   */
  isPartialBackgroundResponseDelivery(aggregation, partCount) {
    return partCount > 0 && (aggregation.delivered === true || aggregation.dropped === true || aggregation.resolutionDropped === true || aggregation.retiring === true || aggregation.completionPartial === true || aggregation.turnComplete !== true);
  }
  /**
   ...
   */
  owesTerminalBackgroundResponseCard(aggregation) {
  ...
  refreshBackgroundResponseDelivery(aggregation) {
    const delivery = aggregation.delivery;
    if (!delivery || aggregation.flushing)
      return;
    ...
    const replaceHeader = /* @__PURE__ */ __name((text) => text.replace(/## (?:✅|❌|⏹️) Agent · [^\n]+/, () => header), "replaceHeader");
    if (plan) {
  ...
  drainBackgroundResponseAggregations(sessionId) {
    ...
    for (const pending of this.detachedPendingBackgroundResponseTerminals) {
      ...
      pending.turnComplete = true;
      pending.completionPartial = true;
      if (pending.held.length > 0 && this.router.getTarget(pending.sessionId) === pending.target) {
        void this.flushDetachedBackgroundResponse("", pending.sessionId, pending, { target: pending.target }).catch((error) => {
          process.stderr.write(`[DingTalk:${this.name}] background response delivery failed during drain: ${sanitizeLogText(error instanceof Error ? error.message : String(error), 300)}
`);
        });
      } else if (pending.held.length > 0) {
        process.stderr.write(`[DingTalk:${this.name}] background response target unavailable during drain; ${pending.held.length} buffered segment(s) discarded
`);
    ...
    for (const aggregation of aggregations) {
      ...
      if (!aggregation.flushing) {
        void this.flushBackgroundResponseAggregation(aggregation.key, aggregation);
      }
    }
  }
  ...
  scheduleBackgroundResponseResolutionRetry(key, sessionId, pending) {
    ...
    pending.retryTimer = setTimeout(() => {
      ...
      void this.retryBackgroundResponseResolution(key, sessionId, pending).catch((error) => {
        process.stderr.write(`[DingTalk:${this.name}] background response target resolution failed: ${sanitizeLogText(error instanceof Error ? error.message : String(error), 300)}
`);
      }).finally(() => {
...
`);
  ...
  async completeBackgroundResponseAggregation(key, aggregation) {
    if (aggregation.delivery) {
      ...
      aggregation.delivery.partial = this.isPartialBackgroundResponseDelivery(aggregation, aggregation.delivery.parts.length) || aggregation.parts.length > 0;
    }
    if (aggregation.timeoutTimer)
      clearTimeout(aggregation.timeoutTimer);
    aggregation.timeoutTimer = void 0;
  ...
  formatBackgroundResponseAggregation(delivery) {
    const icon = delivery.status === "completed" ? "\u2705" : delivery.status === "failed" ? "\u274C" : "\u23F9\uFE0F";
    const label = this.formatBackgroundAgentLabel(delivery.label);
    const header = `## ${icon} Agent \xB7 ${label}${delivery.partial ? "\uFF08\u90E8\u5206\uFF09" : ""}`;
    if (delivery.parts.length === 0)
      return header;
    return `${header}

${delivery.parts.join("\n\n")}`;
  }
  formatBackgroundAgentResponse(text, label) {
    return `## \u{1F916} Agent \xB7 ${this.formatBackgroundAgentLabel(label)}

${text}`;
  }
  formatBackgroundAgentLabel(label) {
  ...
  async onResponseComplete(chatId, text, sessionId, segment) {
    ...
    await this.sendResponseMessage(chatId, outgoingText, sessionId, segment?.sourceLabel, true);
  }
...
`);
      ...
      const processMessage = content.downloadCodes.length > 0 || quoted.media ? this.prepareThenHandleInbound(envelope, async () => {
  ...
  management: {
    fields: [
      ...
      },
      {
        ...
        kind: "boolean",
        description: "Buffer each background Agent turn and send one labeled result instead of labeled segments"
      },
      {

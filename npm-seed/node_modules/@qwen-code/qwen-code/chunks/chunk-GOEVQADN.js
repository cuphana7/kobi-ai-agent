  ...
  return new Promise((resolve7, reject) => {
    new CoreToolScheduler({
      ...
      onAllToolCallsComplete: /* @__PURE__ */ __name(async (completedToolCalls) => {
        ...
        resolve7(completedToolCalls[0].response);
...
var WebTerminalRegistry = class {
  ...
  async create(options) {
    ...
    const handleData = /* @__PURE__ */ __name((data) => {
      ...
      session2.buffer.push(data);
      session2.bufferBytes += Buffer.byteLength(data);
      ...
      for (const listener of session2.outputListeners) listener(data);
    }, "handleData");
    ...
    try {
      ...
      proc = {
        ...
        kill: /* @__PURE__ */ __name(() => spawned.kill(), "kill")
      };
    } catch {
      this.finishCreating(terminalId);
...
function getUserOutputStylesRoot() {
  ...
  return os.homedir() || path6.dirname(Storage.getGlobalQwenDir());
}
...
var LspServerManager = class {
  ...
  async warmupTypescriptServer(handle, force = false) {
    ...
    try {
      const text = fs10.readFileSync(tsFile, "utf-8");
...
var NativeLspService = class {
  ...
  sendDocumentOpen(serverName, handle, uri) {
    ...
    try {
      ...
      debugLogger19.warn(`Failed to resolve file path for ${uri}:`, error);
      return false;
    }
    let text;
    try {
  ...
  async applyTextEdits(uri, edits) {
    ...
    try {
      await fsp.access(filePath, fs11.constants.W_OK);
    } catch (err) {
      if (err?.code !== "ENOENT") {
        throw err;
      }
...
function resolveGitDir(crawlDirectory) {
  ...
  while (current) {
    ...
    try {
      const stat5 = fs13.statSync(gitPath);
      if (stat5.isDirectory()) {
        pathCacheSet(resolveGitDirCache, cacheKey, gitPath);
        return gitPath;
      }
      if (stat5.isFile()) {
        const contents = fs13.readFileSync(gitPath, "utf8").trim();
        ...
        try {
          resolvedAbs = fs13.realpathSync(resolvedAbs);
          try {
            fs13.statSync(path15.join(resolvedAbs, "HEAD"));
          } catch {
            fs13.statSync(path15.join(resolvedAbs, "index"));
          }
        } catch {
          pathCacheSet(resolveGitDirCache, cacheKey, null);
          return null;
...
function getGitRootMtime(crawlDirectory) {
  try {
    ...
    const indexStat = fs13.statSync(indexPath);
...
function getPosixRelative(from, to) {
  ...
  try {
    fromAbs = fs13.realpathSync(fromResolved);
  } catch {
  }
  try {
    toAbs = fs13.realpathSync(toResolved);
  } catch {
  }
...
async function crawlWithGitLsFiles(stateKey, crawlDirectory, cwd, options, workingTreePrefetch) {
  ...
  const processTrackedFile = /* @__PURE__ */ __name((file) => {
    ...
    let stat5;
    try {
      stat5 = fs13.lstatSync(path15.join(gitRoot, ...normalizedFile.split("/")));
    } catch {
      return true;
    }
    if (stat5.isDirectory()) {
...
function getWorkerScriptPath() {
  ...
  resolvedWorkerPath = fs14.existsSync(candidate) ? candidate : null;
...
async function readPathFromWorkspace(pathStr, config) {
  ...
  if (path18.isAbsolute(pathStr)) {
    ...
    for (const dir of searchDirs) {
      ...
      try {
        await fs15.access(potentialPath);
...
async function getWelcomeBackState() {
  try {
    const raw = await fs16.readFile(getWelcomeBackStatePath(), "utf-8");
...
async function saveWelcomeBackRestartChoice(summaryFingerprint) {
  ...
  await fs16.mkdir(path19.dirname(statePath), { recursive: true });
  const state = {
    version: 1,
    lastChoice: "restart",
    summaryFingerprint
...
async function clearWelcomeBackState() {
  try {
    await fs16.rm(getWelcomeBackStatePath(), { force: true });
  } catch {
  }
}
...
async function getProjectSummaryInfo() {
  ...
  try {
    const summaryStat = await fs16.stat(summaryPath);
    const content = await fs16.readFile(summaryPath, "utf-8");
...
async function readManyFiles(config, options) {
  ...
  try {
    ...
    for (const rawPattern of inputPatterns) {
      ...
      const stats = fs17.existsSync(fullPath) ? fs17.statSync(fullPath) : null;
      if (stats?.isDirectory()) {
...
async function matchesValidatedPathIdentity(filePath, expected) {
  try {
    const canonicalPath = await fs17.promises.realpath(filePath);
    if (canonicalPath !== filePath) return false;
    const stats = await fs17.promises.stat(canonicalPath);
...
async function snapshotValidatedFile(filePath, expected, signal) {
  ...
  try {
    ...
    try {
      ...
      snapshotDir = await fs17.promises.mkdtemp(
        path20.join(os3.tmpdir(), "qwen-validated-read-")
      );
      const snapshotPath = path20.join(snapshotDir, path20.basename(filePath));
      const target = await fs17.promises.open(
        snapshotPath,
        fs17.constants.O_WRONLY | fs17.constants.O_CREAT | fs17.constants.O_EXCL,
        384
      );
      try {
      ...
      result = {
        ...
        stats,
        cleanup: /* @__PURE__ */ __name(() => fs17.promises.rm(snapshotDir, { recursive: true, force: true }), "cleanup")
      };
      return result;
    } finally {
      await source.close();
    ...
    if (snapshotDir && !result) {
      await fs17.promises.rm(snapshotDir, { recursive: true, force: true });
    }
  }
}

...
function startPostRenderPrefetches(config, settings, options = {}) {
  ...
  if (settings.merged.general?.enableAutoUpdate !== false) {
    runDeferredTask("update_check", async () => {
      const [{ checkForUpdates }, { handleAutoUpdate }] = await Promise.all([
        import("./updateCheck-KONLM73X.js"),
        import("./handleAutoUpdate-CEZJAARW.js")
      ]);
  ...
  if (options.connectIde && config.getIdeMode()) {
    runDeferredTask("ide_connect", async () => {
      ...
      try {
        const { connectIdeForStartup } = await import("./initializer-BRFJKRRX.js");
        await withTimeout(
  ...
  if (config.isInteractive()) {
    runDeferredTask("background_housekeeping", async () => {
      const { startBackgroundHousekeeping } = await import("./scheduler-O4IGVZKK.js");

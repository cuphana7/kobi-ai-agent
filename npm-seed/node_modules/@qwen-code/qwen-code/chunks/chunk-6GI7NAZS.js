...
function runWithAgentDisallowedTools(disallowedTools, fn) {
  const current = storage.getStore() ?? {};
  return storage.run({ ...current, disallowedTools }, fn);
}
__name(runWithAgentDisallowedTools, "runWithAgentDisallowedTools");
function runWithAgentConfiguredToolAllowlist(configuredToolAllowlist, fn) {
  const current = storage.getStore() ?? {};
  return storage.run({ ...current, configuredToolAllowlist }, fn);
}
__name(runWithAgentConfiguredToolAllowlist, "runWithAgentConfiguredToolAllowlist");
function getCurrentAgentConfiguredToolAllowlist() {
  return storage.getStore()?.configuredToolAllowlist;
}
__name(getCurrentAgentConfiguredToolAllowlist, "getCurrentAgentConfiguredToolAllowlist");
function getCurrentAgentDisallowedTools() {
  return storage.getStore()?.disallowedTools;
}
__name(getCurrentAgentDisallowedTools, "getCurrentAgentDisallowedTools");

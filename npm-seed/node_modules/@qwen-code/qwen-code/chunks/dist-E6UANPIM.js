...
`);
    ...
    try {
      ...
      const resp = await sendQQMessage(route.base, route.path, this.accessToken, body);
      if (!resp.ok) {
        const errBody = await resp.text().catch(() => "");
        process.stderr.write(`[QQ:${this.name}] Send failed (HTTP ${resp.status}: ${sanitizeLogText(errBody, 200)})
`);
        if (resp.status === 429) {
...
`);
        ...
        if (msgId) {
          ...
          const activeResp = await sendQQMessage(route.base, route.path, this.accessToken, activeBody);
          if (activeResp.ok) {
            process.stderr.write(`[QQ:${this.name}] Active retry succeeded for ${sanitizeLogText(chatId, 64)}
`);
          ...
          process.stderr.write(`[QQ:${this.name}] Active retry also failed (HTTP ${activeResp.status}: ${sanitizeLogText(await activeResp.text().catch(() => ""), 200)})
`);
          if (activeResp.status === 429) {
            process.stderr.write(`[QQ:${this.name}] MESSAGE DROPPED: active retry rate-limited (HTTP 429) for ${sanitizeLogText(chatId, 64)}
`);
            this.saveQQState();
            return;
          }
          process.stderr.write(`[QQ:${this.name}] MESSAGE DROPPED: both passive and active send failed for ${sanitizeLogText(chatId, 64)}
`);
          this.saveQQState();
          return;
        }
...
`);
  ...
  onResponseChunk(chatId, chunk, sessionId) {
    ...
    if (!state) {
      ...
      if (state.buffer.length >= _QQChannel.MAX_BUFFER_LENGTH) {
...
`);
  ...
  flushAndTrack(sessionId, buffer, state, logLabel) {
    ...
    this.sendMessage(state.chatId, buffer).then(() => {
...
`);
      if (this.pendingStreamDelete.has(sessionId)) {
        ...
        if (current === state) {
          ...
          current.retryCount++;
          if (current.retryCount < _QQChannel.MAX_FLUSH_RETRIES) {
            const reconnectId = this.reconnectId;
...
`);
        ...
        if (current === state) {
          ...
          if (current.buffer.length >= _QQChannel.MAX_BUFFER_LENGTH) {
            this.idleFlush(sessionId, this.reconnectId);
          } else {
            current.retryCount++;
            if (current.retryCount < _QQChannel.MAX_FLUSH_RETRIES) {
...
`);
      ...
      if (raw.chatTypeMap && Array.isArray(raw.chatTypeMap)) {
        const rawCT = raw.chatTypeMap;
        this.chatTypeMap = new Map(rawCT.filter(([k, v]) => typeof k === "string" && k.length <= 256 && (v === "c2c" || v === "group")));
        const dropped = rawCT.length - this.chatTypeMap.size;
        if (dropped > 0)
          process.stderr.write(`[QQ:${this.name}] Dropped ${dropped} invalid chatTypeMap entries during restore
`);
      }
      if (raw.replyMsgId && Array.isArray(raw.replyMsgId)) {
        const now = Date.now();
        const rawRM = raw.replyMsgId;
        this.replyMsgId = new Map(rawRM.map(([k, v]) => (
          // Old format: string -> { msgId: v, timestamp: now }
          // New format: { msgId, timestamp } -> pass through
          typeof v === "string" ? [k, { msgId: v, timestamp: now }] : [k, v]
        )).filter(([k, v]) => {
          ...
          if (typeof v !== "object" || v === null)
            return false;
          const entry = v;
          return typeof entry.msgId === "string" && entry.msgId.length <= 128 && typeof entry.timestamp === "number" && Number.isFinite(entry.timestamp) && entry.timestamp <= now + _QQChannel.REPLY_MSG_ID_TTL_MS;
        }));
        const dropped = rawRM.length - this.replyMsgId.size;
        if (dropped > 0)
          process.stderr.write(`[QQ:${this.name}] Dropped ${dropped} invalid replyMsgId entries during restore
`);
      }
      if (raw.msgSeqMap && Array.isArray(raw.msgSeqMap)) {
        const rawMS = raw.msgSeqMap;
        this.msgSeqMap = new Map(rawMS.filter(([k, v]) => typeof k === "string" && k.length <= 256 && typeof v === "number" && Number.isSafeInteger(v) && v >= 0));
        const dropped = rawMS.length - this.msgSeqMap.size;
        if (dropped > 0)
          process.stderr.write(`[QQ:${this.name}] Dropped ${dropped} invalid msgSeqMap entries during restore
`);
      }
      return true;
    } catch (e) {
    ...
    if (delay > 0) {
      this.tokenRefreshTimer = setTimeout(() => {
        this.fetchToken().catch((e) => {
          if (this.disposed)
            return;
          process.stderr.write(`[QQ:${this.name}] Token refresh failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}, retrying in 60s
`);
...
`);
        ...
        process.stderr.write(`[QQ:${this.name}] RC: ${sanitizeLogText(msg, 200)} (retry in ${backoff}ms, attempt ${attempt + 1}/${maxGwRetries})
`);

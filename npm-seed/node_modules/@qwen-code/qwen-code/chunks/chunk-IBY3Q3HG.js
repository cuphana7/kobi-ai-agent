...
var DaemonChannelBridge = class extends EventEmitter2 {
  ...
  async prompt(sessionId, text, options) {
    ...
    try {
      ...
      await Promise.race([
        turnBarrier,
        new Promise((resolve2) => setTimeout(resolve2, 0))
      ]);
  ...
  resolveTurnBarrier(sessionId) {
    const resolve2 = this.turnBarriers.get(sessionId);
    if (resolve2) {
...
var SessionRouter = class {
  ...
  async resolve(channelName, senderId, chatId, threadId, cwd, isGroup) {
...
`);
    ...
    this.registerCommand("forget-channel", async (envelope, args) => {
      ...
      await this.handleChannelMemoryIntent(envelope, { kind: "clear_confirm" }, { skipPendingClear: true });
      return true;
    });
...
`);
  ...
  async handleChannelMemoryIntent(envelope, intent, options = {}) {
    ...
    if (intent.kind === "remember") {
      try {
        await channelMemory.appendChannelMemory(this.channelMemoryTarget(envelope), intent.text);
      } catch (error) {
  ...
  async classifyChannelMemoryIntent(text) {
    ...
    let classified;
    try {
      classified = await this.memoryIntentClassifier.classifyChannelMemoryIntent(text);
    } catch (error) {

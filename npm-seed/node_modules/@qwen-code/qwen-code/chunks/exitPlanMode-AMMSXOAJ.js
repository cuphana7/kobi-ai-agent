...
${notes}` : "")
  ...
  async executePlanRequiredTeammate(plan, originalRequest, researchSummary, signal) {
    ...
    if (!teammate || !manager) {
      const message = "Plan-required teammate approval is unavailable in this context.";
      return {
        llmContent: message,
    ...
    if (decision.action === "reject") {
      const feedback2 = decision.message ? `

Leader feedback:
${decision.message}` : "";
      const llmContent = "Leader rejected the plan. Revise the plan based on the feedback and call exit_plan_mode again." + feedback2;
      return {
        llmContent,
        returnDisplay: this.buildRejectedGateDisplay(
    ...
    const feedback = decision.message ? ` Leader note: ${decision.message}` : "";
    return {
  ...
  fallbackToUserDecision(plan) {
    ...
    return {
      ...
      returnDisplay: {
        type: "plan_summary",
        message: "Plan gate is unavailable. The plan has been saved, and plan mode remains active until the user explicitly approves execution.",
        plan
      }
    };
  }

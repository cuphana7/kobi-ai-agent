...
function getConfiguredModelReasoning(config, modelId = config.getModel?.(), fallbackToManifest = true) {
  if (config.getActiveRuntimeModelSnapshot?.() || config.getModel?.().startsWith(ACP_ROUTE_ID_PREFIX)) {
    return void 0;
  }
  const generation = config.getContentGeneratorConfig?.();
  const authType = generation?.authType ?? config.getAuthType?.();
  const baseUrl = generation?.baseUrl ?? config.getCurrentModelRegistryBaseUrl?.() ?? void 0;
  const configured = authType ? config.getResolvedModelConfig?.(authType, modelId, baseUrl) : void 0;

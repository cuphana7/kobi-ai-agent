...
function goalRestoreBlockedBy(config) {
...
__name(goalRestoreBlockedBy, "goalRestoreBlockedBy");
function goalConditionBlockedBy(condition) {
  if (condition.length === 0) return "condition-invalid";
  return null;
}
__name(goalConditionBlockedBy, "goalConditionBlockedBy");
function restoreGoalFromHistory(history, config, addItem) {
  ...
  registerGoalHook({
    ...
    ...restorable.setAt !== void 0 ? { initialSetAt: restorable.setAt } : {}
  });
  if (addItem) {
...
function extractTranscriptTodos(resultDisplay, args) {
  const fromDisplay = extractTodosFromDisplay(resultDisplay);
  if (fromDisplay) return fromDisplay;
  return args && Array.isArray(args["todos"]) ? normalizeTodos(args["todos"]) : null;
}
__name(extractTranscriptTodos, "extractTranscriptTodos");
...
var DefaultTranscriptReplayMachine = class {
  ...
  *projectUserRecord(record, emit, meta) {
    if (record.subtype === "notification" || record.subtype === "cron" || record.subtype === "mid_turn_user_message") {
      const payload = isObjectRecord(record.systemPayload) ? record.systemPayload : void 0;
      const displayText = payload && typeof payload["displayText"] === "string" ? payload["displayText"] : void 0;
      if (displayText) {
        yield emit(
          createTranscriptMessageUpdate({
            role: "user",
...
function extractTodosFromDisplay(value) {
  if (isObjectRecord(value) && value["type"] === "todo_list") {
    return Array.isArray(value["todos"]) ? normalizeTodos(value["todos"]) : null;
  }
  if (typeof value !== "string") return null;
  try {
    const parsed = JSON.parse(value);
    return isObjectRecord(parsed) && parsed["type"] === "todo_list" && Array.isArray(parsed["todos"]) ? normalizeTodos(parsed["todos"]) : null;
  } catch {
    return null;
  }
}
__name(extractTodosFromDisplay, "extractTodosFromDisplay");
...
var HistoryReplayer = class {
  ...
  async replayPage(records, options = {}) {
    ...
    const state = {
      pendingToolCalls: options.finalizeDangling === true ? [] : replay.pendingToolCalls.map(toLegacyPendingToolCall),
      replay
    };
    this.setActiveRecordId(null);
...
async function extractMetadata(conversation, config) {
  ...
  let gitRepo;
  if (cwd) {
    const { getGitRepoName } = await import("./src-43LM7JLB.js");

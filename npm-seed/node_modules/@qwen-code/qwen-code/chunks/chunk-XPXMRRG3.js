...
import {
  isGitRepository
} from "./chunk-IOISNXQ2.js";
import {
  BaseDeclarativeTool,
  BaseToolInvocation
} from "./chunk-5XEZQQG3.js";
import {
  ToolDisplayNames,
  ToolNames
} from "./chunk-RSWOFJBY.js";
import {
  QWEN_DIR,
  createDebugLogger
} from "./chunk-GYAL5QAR.js";
import {
  init_esbuild_shims
...
var PREFER_DEDICATED_TOOLS_PREFIX = "- **Prefer Dedicated Tools:**";
function gateToolGuidance(section, surface) {
  const declared = surface?.declaredTools;
  if (!declared) return section;
  const kept = section.split("\n").filter((line) => {
    const gate = TOOL_GUIDANCE_LINE_GATES.find(
      (entry) => line.startsWith(entry.prefix)
    );
    return !gate || gate.tools.every((tool) => declared.has(tool));
  });
  const parentIndex = kept.findIndex(
    (line) => line.startsWith(PREFER_DEDICATED_TOOLS_PREFIX)
  );
  if (parentIndex !== -1 && !kept[parentIndex + 1]?.startsWith("  - To ") && !kept[parentIndex + 1]?.startsWith("  - Reserve using the")) {
    kept.splice(parentIndex, 1);
  }
  return kept.join("\n");
}
__name(gateToolGuidance, "gateToolGuidance");
var TOOL_CALL_IN_EXAMPLE = /\[tool_call:\s*([A-Za-z0-9_]+)|<function=([A-Za-z0-9_]+)>|"name":\s*"([A-Za-z0-9_]+)",\s*"arguments"|<\|tool_call>call:([A-Za-z0-9_]+)/g;
function exampleToolNames(block) {
  return [...block.matchAll(TOOL_CALL_IN_EXAMPLE)].map(
    (match) => match[1] ?? match[2] ?? match[3] ?? match[4]
  );
}
__name(exampleToolNames, "exampleToolNames");
var EXAMPLE_BLOCK = /<example>[\s\S]*?<\/example>\n*/g;
function filterToolCallExamples(examples, surface) {
  const declared = surface?.declaredTools;
...
__name(filterToolCallExamples, "filterToolCallExamples");
...
function getToolGuidanceSection(questions, codeModeOnly, todoWriteEnabled, surface) {
...
function buildDefaultBasePrompt(interaction, model, outputStyle, todoWriteEnabled = false, codeModeOnly = false, surface) {
...
function getCoreSystemPrompt(userMemory, model, appendInstruction, interactionMode = "interactive", outputStyle, todoWriteEnabled = false, codeModeOnly = false, options) {

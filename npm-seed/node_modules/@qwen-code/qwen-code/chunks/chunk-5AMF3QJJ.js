...
} from "./chunk-N7RPJIQD.js";
import {
  scanForSecrets
} from "./chunk-UEILASYG.js";
import {
...
} from "./chunk-LWQKWO5I.js";
import {
  shellResultText
} from "./chunk-4M7RD5QQ.js";
import {
  hasVerifiableInode,
  openNoFollow
...
} from "./chunk-JBUMBQAO.js";
import {
  flattenPeerLabel
} from "./chunk-AO66AVEJ.js";
import {
  isPidAlive
} from "./chunk-3RCEJIII.js";
import {
  getProjectSkillsRoot,
  parse
} from "./chunk-JXNPH2EP.js";
import {
  glob,
  globSync
} from "./chunk-IQQRNRQV.js";
import {
  disposeConoutWorker,
  loadPty,
  loadXtermHeadless,
...
} from "./chunk-VNTJJ4WQ.js";
import {
  require_picomatch
} from "./chunk-OU26ZZOL.js";
import {
  BUILT_IN_OUTPUT_STYLES,
  getBuiltInOutputStyle
} from "./chunk-XPXMRRG3.js";
import {
  DEFAULT_IMAGE_TOKEN_ESTIMATE,
  estimatePartChars
} from "./chunk-GC3J5W2O.js";
import {
  fetchWithPolicy
} from "./chunk-X37DXS3E.js";
import {
  getModelsForProviderProtocol,
  tryResolveModelProtocol
} from "./chunk-73R6BAOK.js";
import {
...
} from "./chunk-HZU2DMPO.js";
import {
  E_TIMEOUT,
  Mutex,
  withTimeout
} from "./chunk-VBT4BMCQ.js";
import {
  getAutoMemoryRoot,
  getUserAutoMemoryRoot
} from "./chunk-UEYDN2YO.js";
import {
...
} from "./chunk-7JDVJX7A.js";
import {
  runSideQuery
} from "./chunk-EAJHTQOM.js";
import {
...
} from "./chunk-GNGQGEZD.js";
import {
  getQwenIgnoreFileNames,
  require_ignore
} from "./chunk-HDDAI7GZ.js";
import {
  ToolNames,
  canonicalToolName
...
import {
  atomicWriteFile,
  atomicWriteJSON
} from "./chunk-XRL3EFYD.js";
import {
  isBinary,
  normalizeContent,
  stripAnsiAndControl,
  stripHtmlComments
} from "./chunk-5R36APDE.js";
import {
  PATH_ARG_KEYS,
  QWEN_DIR,
  Storage,
...
var McpTransportPool = class {
  ...
  async drainAll(opts) {
    ...
    await Promise.race([
      ...
      new Promise((resolve8) => {
        drainTimer = setTimeout(() => resolve8(), remaining);
        drainTimer.unref?.();
      })
    ]);
...
async function bindAt(requestedPath, options, automaticPath) {
  ...
  const listenAt = /* @__PURE__ */ __name((target) => new Promise((resolve8, reject) => {
...
// packages/core/src/goals/goal-card.ts
init_esbuild_shims();
function projectGoalCard(payload, previousGoal = null) {
...
__name(projectGoalCard, "projectGoalCard");
...
var Logger = class {
  ...
  pendingPurgeSessions = /* @__PURE__ */ new Set();
  debugLogger;
  /**
   ...
   */
  adoptDiskSnapshot(rows, ownRow) {
    this.logs = this.pendingPurgeSessions.size === 0 ? rows : rows.filter(
      (row) => row === ownRow || !this.pendingPurgeSessions.has(row.sessionId)
    );
  }
  async _readLogFile() {
    if (!this.logFilePath) {
   ...
   */
  async removeSessionMessages(sessionId) {
   ...
   * @returns true when rows were actually removed from the file
   */
  async removeSessionsMessages(sessionIds) {
    if (!this.initialized || !this.logFilePath || sessionIds.length === 0) {
      return false;
    }
    const doomed = new Set(sessionIds);
    const belongsToSession = /* @__PURE__ */ __name((e) => doomed.has(e.sessionId), "belongsToSession");
    const rowKey = /* @__PURE__ */ __name((e) => JSON.stringify([
      e.sessionId,
      e.messageId,
      e.timestamp,
      e.message,
    ...
    const droppedUndoTarget = this.lastLoggedUserEntry !== null && belongsToSession(this.lastLoggedUserEntry) ? this.lastLoggedUserEntry : null;
  ...
  async purgeFromDisk(logFilePath, belongsToSession, restoreOptimistic) {
    {
      let currentLogsOnDisk;
      try {
        currentLogsOnDisk = await this._readLogFile();
...
var ClientMcpRegistrar = class {
  ...
  sendSdkMcpMessage = /* @__PURE__ */ __name((serverName, message) => {
    ...
    return new Promise((resolve8, reject) => {
      ...
      timer.unref?.();
      this.pending.set(id, { resolve: resolve8, reject, timer, server: serverName });
      let sendResult;
      try {
        sendResult = this.sendFrame({
          id,
...
var DropReceiptCoalescer = class {
  ...
  async flush(boundMs = DROP_FLUSH_BOUND_MS) {
    ...
    const deadline = new Promise((resolve8) => {
      const timer = setTimeout(resolve8, boundMs);
      timer.unref?.();
    });
    void deadline.then(() => {
      overdue = true;
...
function droppedNoun(kind, interim, count) {
  const singular = kind === "agent" ? "agent result" : kind === "shell" ? "shell result" : kind === "workflow" ? "workflow result" : kind === "cron" ? "scheduled prompt" : kind === "peer" ? "cross-session message" : interim ? "monitor pulse" : "monitor result";
...
function buildUsageDashboard(records, options = {}) {
  ...
  const report2 = aggregateUsage(records, range);
  let inputTokens = 0;
  let outputTokens = 0;
  let cachedTokens = 0;
  let thoughtsTokens = 0;
  let totalTokens = 0;
  for (const m of Object.values(report2.models)) {
  ...
  const summary = {
    ...
    cachedTokens,
    thoughtsTokens,
    requests: report2.totalRequests,
    sessions: report2.sessionCount,
    toolCalls: report2.tools.totalCalls,
    linesAdded: report2.files.linesAdded,
    linesRemoved: report2.files.linesRemoved,
    cacheReadRate: inputTokens > 0 ? cachedTokens / inputTokens : 0
  };
  const now = Date.now();
  const models = Object.entries(report2.models).map(([model, m]) => ({
    model,
    totalTokens: modelTokens(m),
    cacheReadRate: m.inputTokens > 0 ? m.cachedTokens / m.inputTokens : 0,
    share: totalTokens > 0 ? modelTokens(m) / totalTokens : 0
  })).sort((a, b) => b.totalTokens - a.totalTokens);
  const skills = report2.skills.topSkills.map((s) => ({
    name: s.name,
    count: s.count
  }));
...
async function serializeMutation(directory, task) {
  ...
  const current = new Promise((resolve8) => {
    resolveCurrent = resolve8;
  });
...
var JsonRpcConnection = class {
  ...
  sendRequest(method, params) {
    ...
    const requestPromise = new Promise((resolve8, reject) => {
      const timer = setTimeout(() => {
...
init_esbuild_shims();
function resolveTextDocumentSync(sync2) {
  const change = typeof sync2 === "number" ? sync2 : sync2?.change ?? 0;
  const openClose = typeof sync2 === "number" ? sync2 !== 0 : sync2?.openClose ?? false;
...
var LspServerManager = class {
  ...
  async warmupTypescriptServer(handle, synchronizeDocument, force = false) {
    ...
    try {
      ...
      await new Promise(
        (resolve8) => setTimeout(resolve8, DEFAULT_LSP_WARMUP_DELAY_MS)
      );
    ...
    try {
      ...
      try {
        await Promise.race([
          shutdownPromise,
          new Promise((resolve8) => {
            timerId = setTimeout(resolve8, timeout);
  ...
  async raceStartupAbort(promise2, signal, cleanupAfterAbort, failurePromise) {
    if (!signal) {
      if (!failurePromise) {
        return promise2;
      }
      void promise2.catch(() => void 0);
      void failurePromise.catch(() => void 0);
      return Promise.race([promise2, failurePromise]);
    }
    this.throwIfStartupAborted(signal);
    let abortWon = false;
    const observedPromise = promise2.then(
      (value) => {
        if (abortWon) {
  ...
  async waitForSocketProcessSpawn(process3, signal) {
    ...
    await new Promise((resolve8, reject) => {
      ...
      const onSpawn = /* @__PURE__ */ __name(() => settle(resolve8), "onSpawn");
  ...
  async prepareCallHierarchy(location, serverName, limit = 50) {
    ...
    for (const [name, handle] of handles) {
      ...
      try {
        ...
        const normalize2 = /* @__PURE__ */ __name((value) => {
          const items2 = [];
        ...
        const items = normalize2(response);
        ...
        try {
          ...
          observeProducer({
            ...
            values: /* @__PURE__ */ __name(() => [
              ...
              ...shellResultText(result.returnDisplay) !== void 0 ? [
                {
...
import { basename as basename5, dirname as dirname7, normalize, relative as relative2, resolve as resolve6, sep as sep5 } from "path";
import * as nativeFs from "fs";
var __require2 = /* @__PURE__ */ createRequire(import.meta.url);
function cleanPath(path21) {
  let normalized2 = normalize(path21);
  if (normalized2.length > 1 && normalized2[normalized2.length - 1] === sep5) normalized2 = normalized2.substring(0, normalized2.length - 1);
  return normalized2;
}
...
function normalizePath(path21, options) {
  const { resolvePaths, normalizePath: normalizePath$1, pathSeparator } = options;
  const pathNeedsCleaning = process.platform === "win32" && path21.includes("/") || path21.startsWith(".");
  if (resolvePaths) path21 = resolve6(path21);
  if (normalizePath$1 || pathNeedsCleaning) path21 = cleanPath(path21);
  if (path21 === ".") return "";
...
function joinPathWithRelativePath(root, options) {
  return function(filename, directoryPath) {
    ...
    else return convertSlashes(relative2(root, directoryPath), options.pathSeparator) + options.pathSeparator + filename;
  };
}
...
var pushDirectoryFilter = /* @__PURE__ */ __name((directoryPath, paths, filters) => {
  ...
  if (filters.every((filter2) => filter2(path21, true))) paths.push(path21);
}, "pushDirectoryFilter");
...
var resolveSymlinksAsync = /* @__PURE__ */ __name(function(path21, state, callback$1) {
  const { queue, fs: fs17, options: { suppressErrors } } = state;
  queue.enqueue();
  fs17.realpath(path21, (error, resolvedPath) => {
    if (error) return queue.dequeue(suppressErrors ? null : error, state);
    fs17.stat(resolvedPath, (error$1, stat5) => {
      if (error$1) return queue.dequeue(suppressErrors ? null : error$1, state);
      if (stat5.isDirectory() && isRecursive(path21, resolvedPath, state)) return queue.dequeue(null, state);
      callback$1(stat5, resolvedPath);
      queue.dequeue(null, state);
    });
  });
}, "resolveSymlinksAsync");
var resolveSymlinks = /* @__PURE__ */ __name(function(path21, state, callback$1) {
  const { queue, fs: fs17, options: { suppressErrors } } = state;
  queue.enqueue();
  try {
    const resolvedPath = fs17.realpathSync(path21);
    const stat5 = fs17.statSync(resolvedPath);
    if (stat5.isDirectory() && isRecursive(path21, resolvedPath, state)) return;
    callback$1(stat5, resolvedPath);
  } catch (e) {
...
function isRecursive(path21, resolved, state) {
  ...
  let parent = dirname7(path21);
  let depth = 1;
  while (parent !== state.root && depth < 2) {
    const resolvedPath = state.symlinks.get(parent);
    const isSameRoot = !!resolvedPath && (resolvedPath === resolved || resolvedPath.startsWith(resolved) || resolved.startsWith(resolvedPath));
    if (isSameRoot) depth++;
    else parent = dirname7(parent);
  }
...
var walkAsync = /* @__PURE__ */ __name((state, crawlPath, directoryPath, currentDepth, callback$1) => {
  ...
  fs17.readdir(crawlPath || ".", readdirOpts, (error, entries = []) => {
...
var walkSync = /* @__PURE__ */ __name((state, crawlPath, directoryPath, currentDepth, callback$1) => {
  ...
  let entries = [];
  try {
    entries = fs17.readdirSync(crawlPath || ".", readdirOpts);
  } catch (e) {
  ...
  walk = /* @__PURE__ */ __name((entries, directoryPath, depth) => {
    ...
    for (let i = 0; i < entries.length; ++i) {
      ...
      if (entry.isFile() || entry.isSymbolicLink() && !resolveSymlinks$1 && !excludeSymlinks) {
        ...
        let path21 = joinDirectoryPath(entry.name, directoryPath, this.state.options.pathSeparator);
        if (exclude && exclude(entry.name, path21)) continue;
        this.pushDirectory(path21, paths, filters);
        this.walkDirectory(this.state, path21, path21, depth - 1, this.walk);
      } else if (this.resolveSymlink && entry.isSymbolicLink()) {
        let path21 = joinPathWithBasePath(entry.name, directoryPath);
        this.resolveSymlink(path21, this.state, (stat5, resolvedPath) => {
          if (stat5.isDirectory()) {
            resolvedPath = normalizePath(resolvedPath, this.state.options);
            if (exclude && exclude(entry.name, useRealPaths ? resolvedPath : path21 + pathSeparator)) return;
            ...
            const directoryPath$1 = normalizePath(dirname7(resolvedPath), this.state.options);
...
function runCommand(command, args, cwd, timeoutMs = 2e4, options) {
  ...
  return new Promise((resolve8) => {
    ...
    const finalize = /* @__PURE__ */ __name((success, resultLines) => {
      ...
      settled = true;
      resolve8({ success, lines: resultLines });
    }, "finalize");
    let stderrBuf = "";
    let child;
    try {
...
function getPosixRelative(from, to) {
  ...
  const relative3 = path15.relative(fromAbs, toAbs);
  const posixRel = toPosixPath(relative3);
...
async function findGitRoot(dir) {
  ...
  return normalizePath2(result.lines[0]);
}
...
async function listUntrackedFiles(gitRoot, relativeToGitRoot, useGitignore) {
  ...
  return untrackedResult.lines.map((file) => normalizePath2(file));
}
...
async function listDeletedTrackedFiles(gitRoot, relativeToGitRoot) {
  ...
  return deletedResult.lines.map((file) => normalizePath2(file));
}
...
async function crawlWithGitLsFiles(stateKey, crawlDirectory, cwd, options, workingTreePrefetch) {
  ...
  const processTrackedFile = /* @__PURE__ */ __name((file) => {
    ...
    const normalizedFile = normalizePath2(parsed.filePath);
...
async function crawlWithFdir(options) {
  ...
  try {
    const dirFilter = options.ignore.getDirectoryFilter();
    const fileFilter = options.ignore.getFileFilter();
    const api = new Builder().withRelativePaths().withDirs().withPathSeparator("/").exclude((_excludedName, dirPath) => {
...
var fuzzyMatchV2 = /* @__PURE__ */ __name((caseSensitive, normalize2, forward, input, pattern, withPos, slab2) => {
  ...
  if (slab2 !== null && N * M > slab2.i16.length) {
    return fuzzyMatchV1(caseSensitive, normalize2, forward, input, pattern, withPos);
  }
...
function calculateScore(caseSensitive, normalize2, text, pattern, sidx, eidx, withPos) {
...
var fuzzyMatchV1 = /* @__PURE__ */ __name((caseSensitive, normalize2, forward, text, pattern, withPos, slab2) => {
  if (pattern.length === 0) {
...
var exactMatchNaive = /* @__PURE__ */ __name((caseSensitive, normalize2, forward, text, pattern, withPos, slab2) => {
  if (pattern.length === 0) {
    return [{ start: 0, end: 0, score: 0 }, null];
  }
  const lenRunes = text.length;
  ...
  if (bestPos >= 0) {
    ...
    const [score] = calculateScore(caseSensitive, normalize2, text, pattern, sidx, eidx, false);
...
var buildPatternForBasicMatch = /* @__PURE__ */ __name((query, casing, normalize2) => {
    ...
    return {
      ...
      async find(pattern, limit) {
        ...
        return new Promise((resolve8, reject) => {
          ...
          pending.set(reqId, { resolve: resolve8, reject });
          try {

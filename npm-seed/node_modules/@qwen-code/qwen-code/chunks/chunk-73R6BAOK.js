...
var ModelRegistry = class _ModelRegistry {
  ...
  getModelsForAuthType(authType) {
    ...
    if (!models) return [];
    return Array.from(models.values()).filter((model) => !model.realtimeOnly).map((model) => ({
      id: model.id,
      label: model.name,
...
function buildInstallPlan(config, inputs, existingModels = [], selection) {
  ...
  if (config.id === "custom-openai-compatible" && existingModels?.some(
    (entry) => providerOwns?.(entry) && (entry.imageOnly || entry.voiceOnly || entry.realtimeOnly) && models.some((model) => model.id === entry.id) && typeof entry.baseUrl === "string" && entry.baseUrl !== inputs.baseUrl && resolveEnvKey(config, { ...inputs, baseUrl: entry.baseUrl }) === envKey
  )) {
  ...
  const selectedModels = preserveSavedApis && selection?.id ? models.filter(
    (model) => !model.imageOnly && !model.voiceOnly && !model.realtimeOnly && model.id === selection.id && (!selection.baseUrl || model.baseUrl === selection.baseUrl)
  ) : [];
  const firstModel = selectedModels.find(
    (model) => resolveModelProtocol(savedProtocol, model) === selection?.authType
  ) ?? selectedModels[0] ?? models.find(
    (model) => !model.imageOnly && !model.voiceOnly && !model.realtimeOnly
  );
  if (models.length === 0) {
    throw new Error(
...
async function applyProviderInstallPlan(plan, options) {
  ...
  const preserveSelection = serviceOnly.length > 0 && serviceOnly.every(
    (model) => model.imageOnly || model.voiceOnly || model.realtimeOnly
  );
  ...
  for (const patch of plan.modelProviders ?? []) {
    ...
    const changesRole = existingModels.some(
      (existing) => patch.models.some(
        (model) => (isSameModelIdentity(existing, model) || existing.id === model.id && patch.ownsModel?.(existing) && (preserveSelection || patch.mergeStrategy !== "append")) && (Boolean(existing.imageOnly) !== Boolean(model.imageOnly) || Boolean(existing.voiceOnly) !== Boolean(model.voiceOnly) || Boolean(existing.realtimeOnly) !== Boolean(model.realtimeOnly) || isImageGenerationCapable(existing) && !isImageGenerationCapable(model))
      )
    );
    const removesConversation = preserveSelection && ownBucketModels.some(
      (existing) => !existing.imageOnly && !existing.voiceOnly && !existing.realtimeOnly && (patch.ownsModel?.(existing) ?? patch.models.some((model) => isSameModelIdentity(existing, model)))
    );
    const removesService = !preserveSelection && patch.mergeStrategy !== "append" && ownBucketModels.some(
      (existing) => (existing.imageOnly || existing.voiceOnly || existing.realtimeOnly) && patch.ownsModel?.(existing) && !patch.models.some((model) => model.id === existing.id)
    );

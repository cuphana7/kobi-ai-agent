...
| :--- | :--- | :--- | :--- | :--- |
| **v20260922** | 2026-09-22 | 배포 옵션 추가 / 코어 업데이트 | - **CLI 전용 배포판(`--cli-only`) 추가** — `./make.sh --cli-only [version]`으로 jennifer-monitor/frism-cm/office-edit/project-bootstrap 4개 스킬과 Computer Use 드라이버를 제외한 코어 Qwen Code CLI만 담은 `Kobi_Installer_CLI_v<version>.zip`을 생성<br>- CLI 전용 설정 변형 `settings.cli-only.json`(`tools.computerUse.enabled: false`) 및 `QWEN.cli-only.md`(Computer Use 관련 지침 절 제거) 추가 — 한국어 응답, 보안 지침(자격증명 파일 차단 등) 게이트는 기존과 동일하게 유지<br>- `Install-Kobi.ps1`의 `[2/3]` 단계 안내 문구를 중립화(`"사용자 설정 배포 및 확장 기능(스킬 · Computer Use) 설치"`)하고, 스킬 4종 각각에 Computer Use와 동일한 패턴의 "안내: OOO 스킬 자산이 없어 설치를 건너뜁니다." 메시지를 추가 — CLI 전용 설치에서도 실제로 설치된 항목만 로그에 정확히 남도록 수정 (부수적으로 자산이 없을 때 빈 스킬 폴더가 생성되던 것도 방지)<br>- `Uninstall-Kobi.ps1`은 모든 정리 항목이 이미 `Test-Path` 가드로 안전하게 동작함을 확인 — 변경 없음<br>- Qwen Code 코어 모듈 0.20.0 → **0.24.3** 업데이트 및 오프라인 캐시(`npm-cache`) / `.tgz`(`Kobi_Installer/assets/pkg/`) 갱신 | Claude Sonnet 5 / 개발지원팀 |

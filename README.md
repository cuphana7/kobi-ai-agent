...
| **v20260917** | 2026-09-17 | 버전 업데이트 (Desktop 갱신) | - **Kobi Desktop(GUI) 최신 재빌드** — 상위 Qwen Code Desktop 소스(`desktop-latest`)로 `Kobi-Desktop-x64.exe` 재생성<br>- 내장 CLI 런타임 및 오프라인 코어 패키지 **qwen-code 0.23.2 → 0.24.0** 업데이트 (`assets/pkg` tarball·`npm-seed`·오프라인 `npm-cache` 갱신) — Desktop 내장 런타임은 `QWEN_CODE_TARBALL`로 CLI와 동일 버전 vendoring<br>- 빌드 환경: bun 1.4.2 + WineHQ 11.0 stable(NSIS 언인스톨러 생성 필수, Ubuntu 기본 wine 9.0은 힙 크래시로 실패) + electron 39.2.7<br>- Desktop 설치물 SHA-256 갱신(`Kobi-Desktop-x64.exe.sha256`)<br>- ⚠️ Linux 크로스빌드 산출물은 미서명·내부 exe electron 기본 아이콘(설치 프로그램 아이콘은 Kobi). 완전 브랜딩·서명은 Windows/CI 빌드 필요<br>- ⚠️ 선행 검증 필요: 실제 Windows PC에서 설치·실행하여 창 제목/About/아이콘 및 사내 vLLM 연결 확인 후 배포 | Claude Opus 4.8 / 개발지원팀 |
